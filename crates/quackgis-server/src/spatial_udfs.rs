...
use datafusion::logical_expr::{
    create_udf, ScalarFunctionArgs, ScalarUDF, ScalarUDFImpl, Signature, TypeSignature, Volatility,
};
...
use sedona_geometry::wkb_factory::wkb_rect;
    ...
    fn update_batch(&mut self, values: &[ArrayRef]) -> DFResult<()> {
        ...
        for i in 0..arr.len() {
            ...
            if let Some(parsed) = crate::mvt::parse_wkb(arr.value(i)) {
                if let Some((geom_type, commands)) = crate::mvt::encode_mvt_geometry(&parsed) {
