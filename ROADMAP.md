...
| Security | SCRAM, outer read/write table allowlists, immutable table/operation RBAC, role-filtered maintained metadata, HS256 JWT role mapping with transaction-local claims and atomic key-file hot rotation, owner-only REST authenticator-password rotation with automatic reconnect and old-password denial, role-aware OpenAPI, actual-process required-TLS/restart rotation, and packaged mTLS/edge-key rotation with old-certificate denial | no RLS, mutable administration, packaged database/JWT rotation, or production revocation drill |
| PostgreSQL catalogs/RBAC | relational core catalogs, development-gated stable user-object/default/comment/NOT-NULL identity, truthfully empty index projection, bounded role-aware spatial metadata, immutable roles/sessions/grants/inquiry, role-aware information schema, packaged psycopg copied-data/COPY/reopen, and packaged OGR SQL-result copied-data readback | upstream identity bundle plus unavailable DuckLake primary/unique/foreign-key indexes, authoritative CRS/subtype metadata, copied-data psql/QGIS, and OGR direct discovery/COPY/no-FID qualification remain open |
| REST | signed-JWT read-only PostgREST-style subset through an authenticator pgwire identity, transaction-local effective role/context, and automatically revalidated per-role catalog/OpenAPI cache; consumes shared monotonic schema/security epochs and connection generations when the durable identity capability exists, with an exact role-filtered revision fallback | signed runtime awaits the official identity bundle; not routed through the tiny client or packaged in Kind |
