...
| Security | SCRAM, outer read/write table allowlists, immutable table/operation RBAC, role-filtered maintained metadata, HS256 JWT role mapping with transaction-local claims and atomic key-file hot rotation, role-aware OpenAPI, actual-process required-TLS/restart rotation, and packaged mTLS/edge-key rotation with old-certificate denial | no RLS, mutable administration, authenticator/database rotation, packaged JWT rotation, or production revocation drill |
| PostgreSQL catalogs/RBAC | relational core catalogs, development-gated stable user-object/default/comment/NOT-NULL identity, truthfully empty index projection, bounded role-aware spatial metadata, immutable roles/sessions/grants/inquiry, role-aware information schema, and a packaged psycopg copied-data/COPY/reopen workflow | upstream identity bundle plus unavailable DuckLake primary/unique/foreign-key indexes, authoritative CRS/subtype metadata, and copied-data psql/OGR/QGIS qualification remain open |
| REST | signed-JWT read-only PostgREST-style subset through an authenticator pgwire identity, transaction-local effective role/context, and automatically revalidated per-role catalog/OpenAPI cache | uses an exact per-request role-filtered revision rather than a shared monotonic epoch; not routed through the tiny client or packaged in Kind |
...
| Iroh cluster edge | shared `control/1`/`edge/1` protocol plus executable config-backed bootstrap, one registered credential/worker lease, challenged worker authentication, typed pgwire/cancellation streams, and a bounded tiny client differentially pass direct TCP plus direct, forced-custom-relay, and public-default-relay iroh result/type/error, simple/extended, parameter, portal, spatial, transaction/disconnect, COPY atomicity, cancellation/quarantine, concurrent-session, and reconnect oracles. K0 packages one mutual-TLS direct-path client with loopback-only backend, ordered replacement, denial, and key-rotation evidence | durable pairing/control state, gossip, worker pools, HTTP transport, packaged resource/hosted-relay operations, and serverless client remain open |
