...
| Transactions | independent sessions, commit/rollback/isolation, failed-transaction `25P02` precedence, harmless idle transaction end, QGIS-style failed COMMIT/ROLLBACK cleanup, cancellable pre-commit writes, and a non-cancellable indeterminate-failure commit boundary | commit response-loss reconciliation remains a Local 1.0 operations gate |
...
| PostgreSQL catalogs/RBAC | PostgreSQL 18.4 startup/version/recovery identity, relational core catalogs, pinned stable user-object/default/comment/NOT-NULL identity, truthfully empty index projection, bounded role-aware spatial metadata, immutable roles/sessions/grants/inquiry, role-aware information schema, packaged psycopg copied-data/COPY/reopen, and packaged OGR SQL-result copied-data readback | direct psql/OGR/QGIS query-shape closure remains; DuckLake key indexes and authoritative CRS metadata remain upstream gaps |
