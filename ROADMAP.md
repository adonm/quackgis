...
| Native bundle | one validated candidate manifest now exact-pins common DuckDB/DuckLake/Spatial source, tree, core-target, ordered patch, toolchain, selected artifact, test, and output identity; the existing exact DuckDB library, signed Spatial artifact, and source-built patched DuckLake are packaged with no online install | source preparation, builds, and artifact acceptance still use separate paths; N0 must complete the central build, common trust/package provenance, and upgrade/rollback unit before S0/Q0 |
