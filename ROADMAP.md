...
| PostgreSQL catalogs/RBAC | PostgreSQL 18.4 startup/version/recovery identity, relational core catalogs, pinned stable user-object/default/comment/NOT-NULL identity, truthfully empty index projection, bounded role-aware spatial metadata, immutable roles/sessions/grants/inquiry, role-aware information schema, packaged psycopg COPY/reopen, complete psql 18.3 `\d+`, OGR SQL-result/direct Point/NULL discovery, and pinned QGIS 3.44.11 query-layer binary reads | DuckLake primary/unique/foreign-key indexes and authoritative CRS metadata remain upstream gaps; OGR-authored COPY, direct-table QGIS after real key support, and broader QGIS filter/identify/render remain |
