...
| REST | signed-JWT read-only PostgREST-style subset through an authenticator pgwire identity, transaction-local effective role/context, and per-role catalog/OpenAPI cache | cache invalidation is manual rather than epoch-driven; not routed through the tiny client or packaged in Kind |
