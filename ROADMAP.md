...
| Security | SCRAM, outer read/write table allowlists, immutable table/operation RBAC, role-filtered maintained metadata, and actual-process required-TLS/restart-rotation evidence | no RLS, mutable administration, role-aware OpenAPI, or packaged rotation/revocation drills |
...
| Iroh cluster edge | shared `control/1`/`edge/1` protocol plus executable config-backed bootstrap, one registered credential/worker lease, challenged worker authentication, typed pgwire/cancellation streams, and a bounded loopback tiny client pass local-direct DuckDB simple/extended, spatial, COPY, rollback, cancellation/quarantine, and reconnect oracles | broader pgwire parity, public/custom relay and resource evidence, packaged ingress, durable pairing/control state, gossip, worker pools, and serverless client remain open |
