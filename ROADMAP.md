...
| Native bundle | one validated candidate manifest exact-pins common DuckDB/DuckLake/Spatial source, tree, core-target, ordered patch, toolchain, selected artifact, test, and output identity; all exact sources prepare with verified result trees and no extension-owned core checkout; bootstrap/build/static/package consumers use that authority and runtime manifests carry its path-free identity; deterministic SPDX/license outputs are packaged; the existing exact DuckDB library, signed Spatial artifact, and source-built patched DuckLake load with no online install | artifacts are still built/accepted through separate paths and the manifest marks that legacy provenance; Spatial transitive license materials remain explicitly incomplete; N0 must complete upstream adoption review, the central build, complete redistribution evidence, and upgrade/rollback before S0/Q0 |
