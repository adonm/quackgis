...
| Native bundle | one validated candidate manifest exact-pins common DuckDB/DuckLake/Spatial source, tree, core-target, ordered patch, toolchain, selected artifact, test, and output identity; all exact sources prepare with verified result trees and no extension-owned core checkout; bootstrap/build/static/package consumers use that authority and runtime manifests carry its path-free identity; the existing exact DuckDB library, signed Spatial artifact, and source-built patched DuckLake are packaged with no online install | artifacts are still built/accepted through separate paths and the manifest marks that legacy provenance; N0 must complete the central build, generated SBOM/license package, and upgrade/rollback unit before S0/Q0 |
