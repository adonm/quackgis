...
| Security | SCRAM, outer read/write table allowlists, immutable table/operation RBAC, role-filtered maintained metadata, HS256 JWT role mapping with transaction-local claims, role-aware OpenAPI, actual-process required-TLS/restart rotation, and packaged mTLS/edge-key rotation with old-certificate denial | no RLS, mutable administration, JWT/database rotation, or production revocation drill |
