...
DuckDBManager::OnPostInit(duckdb::ClientContext &context) {
	...
	// allow_unsigned_extensions is set in Initialize() before instance creation.
	...
	// QUACKGIS: Load sedonadb spatial extension. Provides 254 ST_* functions
	// on BLOB (WKB) geometry, backed by Rust/GeoRust kernels (faster than
	// DuckDB spatial's C++ impl), GEOS topology, PROJ CRS, GDAL raster, and
	// the literal Apache SedonaDB kernel bridge.
	//
	// We do NOT load DuckDB's built-in spatial extension because:
	// 1. Function name conflicts (st_asmvt, st_area, etc.)
	// 2. sedonadb implementations are faster (vectorized Rust)
	// 3. sedonadb uses BLOB (raw WKB) which pg_ducklake translates naturally
	//    from our PG bytea geometry type
	try {
		DuckDBQueryOrThrow(context, "LOAD sedonadb;");
		elog(LOG, "QuackGIS: sedonadb spatial extension loaded");
	} catch (const std::exception &e) {
		elog(WARNING, "QuackGIS: sedonadb extension not loaded: %s",
		     DuckDBErrorMessage(e).c_str());
	}
