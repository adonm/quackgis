...
impl DuckLakeTableWriter {
    ...
    fn begin_write_with_setup(
        ...
        setup: WriteSetupResult,
        object_path_str: String,
    ) -> Result<TableWriteSession> {
        let schema_with_ids =
    ...
    fn begin_pending_data_file_write(
        &self,
        schema_name: &str,
        table_name: &str,
        arrow_schema: &Schema,
        ...
        let setup = self
            .metadata
            .begin_data_file_staging(schema_name, table_name, &columns)?;
    ...
    /// Write batches as a Parquet data file and return its catalog metadata
    /// without committing any DuckLake metadata.
    ///
    /// This is the object-staging half of native table mutations: callers write
    /// replacement/append rows durably first, then pass the returned
    /// [`DataFileInfo`] into [`MetadataWriter::commit_table_mutation`] with any
    /// positional delete files or retirements. If that metadata commit fails,
    /// this object is an orphan for cleanup rather than a partially-visible row
    /// generation.
    pub async fn write_pending_data_file(
        &self,
        schema_name: &str,
        table_name: &str,
        batches: &[RecordBatch],
        ...
        let arrow_schema = batches[0].schema();
        let mut session =
            self.begin_pending_data_file_write(schema_name, table_name, &arrow_schema)?;

        for batch in batches {
            session.write_batch(batch)?;
        }

        session.finish_data_file().await
    }
...
impl TableWriteSession {
    ...
    async fn finish_data_file_inner(&mut self) -> Result<DataFileInfo> {
    ...
    /// Finalize and upload the Parquet data file, returning the metadata needed
    /// for a later atomic table mutation, without publishing catalog metadata.
    pub async fn finish_data_file(mut self) -> Result<DataFileInfo> {
        self.finish_data_file_inner().await
    }

    pub async fn finish(mut self) -> Result<WriteResult> {
        let file_info = self.finish_data_file_inner().await?;
