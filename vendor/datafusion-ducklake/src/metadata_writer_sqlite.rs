...
async fn ensure_mutation_targets_live_sqlite(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    table_id: i64,
    base_snapshot: i64,
    ...
    for data_file_id in &mutation.retire_data_file_ids {
        ...
        let concurrent_delete: Option<i64> = sqlx::query_scalar(
            "SELECT delete_file_id FROM ducklake_delete_file
             WHERE data_file_id = ? AND table_id = ? AND end_snapshot IS NULL AND begin_snapshot > ?",
        )
        .bind(*data_file_id)
        .bind(table_id)
        .bind(base_snapshot)
...
async fn apply_table_mutation_sqlite(
    tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>,
    table_id: i64,
    snapshot_id: i64,
    mutation: &TableMutation,
    ...
    if retired_records != 0 || retired_bytes != 0 {
        sqlx::query(
            "UPDATE ducklake_table_stats
             SET record_count = MAX(0, record_count - ?),
    ...
    for delete in &mutation.delete_files {
        if let Some(prev) = delete.expected_prev_delete_file {
            sqlx::query(
                "UPDATE ducklake_delete_file SET end_snapshot = ?
                 WHERE delete_file_id = ? AND table_id = ? AND end_snapshot IS NULL",
            )
            .bind(snapshot_id)
            .bind(prev)
            .bind(table_id)
...
impl MetadataWriter for SqliteMetadataWriter {
    ...
    fn commit_table_mutation(
        ...
        block_on(async {
            ...
            // `insert_snapshot`); table mutations are DML and carry
            // `schema_version` forward. Any early conflict rolls this row back.
            let (snapshot_id, _schema_version) = insert_snapshot(&mut tx).await?;
            let retired =
                ensure_mutation_targets_live_sqlite(&mut tx, table_id, base_snapshot, mutation)
                    .await?;
            apply_table_mutation_sqlite(&mut tx, table_id, snapshot_id, mutation, &retired).await?;
