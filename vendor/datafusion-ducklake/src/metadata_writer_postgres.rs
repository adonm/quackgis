...
use crate::metadata_writer::{
    ColumnDef, CommitIds, DataFileInfo, DeleteFileMutation, MetadataWriter, TableMutation,
    WriteMode, WriteSetupResult, columns_differ, validate_name,
};
use sqlx::Row;
...
async fn ensure_mutation_targets_live_postgres(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    table_id: i64,
    base_snapshot: i64,
    mutation: &TableMutation,
) -> Result<Vec<(i64, i64, i64)>> {
    for delete in &mutation.delete_files {
        let row = sqlx::query(
            "SELECT begin_snapshot FROM ducklake_data_file
             WHERE data_file_id = $1 AND table_id = $2 AND end_snapshot IS NULL",
        )
        .bind(delete.data_file_id)
        .bind(table_id)
        .fetch_optional(&mut **tx)
        .await?;
        let begin_snapshot: i64 = match row {
            Some(row) => row.try_get(0)?,
            None => {
                return Err(crate::DuckLakeError::Conflict(format!(
                    "delete targets data file {}, which is not live for table {} or was retired; retry against the new generation",
                    delete.data_file_id, table_id
                )));
            },
        };
        if begin_snapshot > base_snapshot {
            return Err(crate::DuckLakeError::Conflict(format!(
                "delete targets data file {}, which was created after base snapshot {}; retry against the new generation",
                delete.data_file_id, base_snapshot
            )));
        }
        ...
        .fetch_optional(&mut **tx)
        .await?;
        if current_prev != delete.expected_prev_delete_file {
            return Err(crate::DuckLakeError::Conflict(format!(
                "delete on data file {} conflicts with a concurrent delete (expected live delete file {:?}, found {current_prev:?}); retry against the new generation",
                delete.data_file_id, delete.expected_prev_delete_file
            )));
        }
    }

    let mut retired = Vec::with_capacity(mutation.retire_data_file_ids.len());
    for data_file_id in &mutation.retire_data_file_ids {
        let row = sqlx::query(
            "SELECT begin_snapshot, COALESCE(record_count, 0), file_size_bytes
        ...
        let Some(row) = row else {
            return Err(crate::DuckLakeError::Conflict(format!(
                "retire targets data file {data_file_id}, which is not live for table {table_id}; retry against the new generation"
            )));
        };
        let begin_snapshot: i64 = row.try_get(0)?;
        if begin_snapshot > base_snapshot {
            return Err(crate::DuckLakeError::Conflict(format!(
                "retire targets data file {data_file_id}, which was created after base snapshot {base_snapshot}; retry against the new generation"
            )));
        }
        let concurrent_delete: Option<i64> = sqlx::query_scalar(
            "SELECT delete_file_id FROM ducklake_delete_file
             WHERE data_file_id = $1 AND table_id = $2 AND end_snapshot IS NULL AND begin_snapshot > $3",
        )
        .bind(*data_file_id)
        .bind(table_id)
        .bind(base_snapshot)
        .fetch_optional(&mut **tx)
        .await?;
        if concurrent_delete.is_some() {
            return Err(crate::DuckLakeError::Conflict(format!(
                "retire of data file {data_file_id} conflicts with a concurrent delete committed since snapshot {base_snapshot}; retry against the new generation"
            )));
        }
        retired.push((*data_file_id, row.try_get(1)?, row.try_get(2)?));
    }

    Ok(retired)
}

async fn apply_table_mutation_postgres(
    tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    table_id: i64,
    snapshot_id: i64,
    mutation: &TableMutation,
    ...
    for (data_file_id, _, _) in retired {
        sqlx::query(
            "UPDATE ducklake_delete_file SET end_snapshot = $1
             WHERE table_id = $2 AND data_file_id = $3 AND end_snapshot IS NULL",
        )
        .bind(snapshot_id)
        .bind(table_id)
        .bind(*data_file_id)
    ...
    let retired_records: i64 = retired.iter().map(|(_, records, _)| *records).sum();
    let retired_bytes: i64 = retired.iter().map(|(_, _, bytes)| *bytes).sum();
    if retired_records != 0 || retired_bytes != 0 {
        sqlx::query(
            "UPDATE ducklake_table_stats
             SET record_count = GREATEST(0::BIGINT, record_count - $1),
                 file_size_bytes = GREATEST(0::BIGINT, file_size_bytes - $2)
             WHERE table_id = $3",
        )
        .bind(retired_records)
        .bind(retired_bytes)
        .bind(table_id)
        .execute(&mut **tx)
        .await?;
    ...
    for delete in &mutation.delete_files {
        if let Some(prev) = delete.expected_prev_delete_file {
            sqlx::query(
                "UPDATE ducklake_delete_file SET end_snapshot = $1
                 WHERE delete_file_id = $2 AND table_id = $3 AND end_snapshot IS NULL",
            )
            .bind(snapshot_id)
            .bind(prev)
            .bind(table_id)
        ...
        .bind(delete.data_file_id)
        .bind(table_id)
        .bind(&delete.delete.path)
        .bind(delete.delete.path_is_relative)
        .bind(delete.delete.file_size_bytes)
        .bind(delete.delete.footer_size)
        .bind(delete.delete.delete_count)
        .bind(snapshot_id)
        .execute(&mut **tx)
        .await?;
    }

    if !mutation.append_data_files.is_empty() {
        let stats_row =
            sqlx::query("SELECT next_row_id FROM ducklake_table_stats WHERE table_id = $1")
                .bind(table_id)
                .fetch_one(&mut **tx)
                .await?;
        let mut next_row_id: i64 = stats_row.try_get(0)?;
        let mut appended_records = 0_i64;
        let mut appended_bytes = 0_i64;
        for file in &mutation.append_data_files {
            sqlx::query(
            ...
            appended_records += file.record_count;
            appended_bytes += file.file_size_bytes;
        }
        sqlx::query(
            "UPDATE ducklake_table_stats
...
impl MetadataWriter for PostgresMetadataWriter {
    ...
    fn commit_table_mutation(
        ...
        block_on(async {
            ...
            assert_table_in_catalog(self.catalog_id, table_id, &mut tx).await?;
            let retired =
                ensure_mutation_targets_live_postgres(&mut tx, table_id, base_snapshot, mutation)
                    .await?;
            let snapshot_id = insert_data_snapshot(self.catalog_id, &mut tx).await?;
            apply_table_mutation_postgres(&mut tx, table_id, snapshot_id, mutation, &retired)
                .await?;
    ...
    fn begin_data_file_staging(
        ...
        block_on(async {
            ...
            .ok_or_else(|| {
                crate::DuckLakeError::InvalidConfig(format!(
                    "pending data file staging requires existing schema '{schema_name}'"
                ))
            })?
            .try_get(0)?;
            ...
            .ok_or_else(|| {
                crate::DuckLakeError::InvalidConfig(format!(
                    "pending data file staging requires existing table '{schema_name}.{table_name}'"
                ))
            })?
            .try_get(0)?;
            ...
            .fetch_all(&mut *tx)
            .await?;
            let mut existing_columns = Vec::with_capacity(rows.len());
            let mut column_ids = Vec::with_capacity(rows.len());
            for row in rows {
                existing_columns.push((
                    row.try_get::<String, _>(0)?,
                    row.try_get::<String, _>(1)?,
                    row.try_get::<Option<bool>, _>(2)?.unwrap_or(true),
                ));
                column_ids.push(row.try_get::<i64, _>(3)?);
            }
            if existing_columns.is_empty() || columns_differ(&existing_columns, columns) {
                return Err(crate::DuckLakeError::InvalidConfig(format!(
                    "pending data file staging for '{schema_name}.{table_name}' must match the current table schema"
                )));
            }
