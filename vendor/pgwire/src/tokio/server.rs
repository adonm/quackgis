...
pub struct PgWireMessageServerCodec<S> {
    ...
    #[new(value = "crate::messages::LARGE_PACKET_SIZE_LIMIT")]
    max_frontend_message_length: usize,
}
...
impl<S> Decoder for PgWireMessageServerCodec<S> {
    ...
    fn decode(&mut self, src: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        let length_offset = match self.client_info.state() {
        ...
        validate_frontend_message_length(src, length_offset, self.max_frontend_message_length)?;

        PgWireFrontendMessage::decode(src, &self.decode_context)
    }
}

/// Validate a declared frontend message length from the available header bytes.
///
/// This function does not reserve or read the declared body.
pub fn validate_frontend_message_length(
    src: &bytes::BytesMut,
    length_offset: usize,
    max_frontend_message_length: usize,
) -> PgWireResult<()> {
    if let Some(message_length) = crate::messages::codec::get_length(src, length_offset)
        && message_length > max_frontend_message_length
    {
...
pub async fn negotiate_tls<S>(
    ...
    negotiate_tls_with_frontend_limit(
        tcp_socket,
        tls_acceptor,
        crate::messages::LARGE_PACKET_SIZE_LIMIT,
    )
    .await
}

async fn negotiate_tls_with_frontend_limit<S>(
    tcp_socket: TcpStream,
    tls_acceptor: Option<crate::tokio::TlsAcceptor>,
    max_frontend_message_length: usize,
) -> io::Result<Option<Framed<MaybeTls, PgWireMessageServerCodec<S>>>> {
    ...
    let mut codec = PgWireMessageServerCodec::new(client_info);
    codec.max_frontend_message_length = max_frontend_message_length;
    let mut tcp_socket = Framed::new(tcp_socket, codec);
    ...
    if let Some(tls_acceptor) = tls_acceptor {
        ...
        let mut codec = PgWireMessageServerCodec::new(client_info);
        codec.max_frontend_message_length = max_frontend_message_length;
        let mut parts = FramedParts::new(MaybeTls::Tls(ssl_socket), codec);
        parts.read_buf = old_parts.read_buf;
        parts.write_buf = old_parts.write_buf;
        let mut socket = Framed::from_parts(parts);
...
where
    H: PgWireServerHandlers,
{
    process_socket_with_frontend_limit(
        tcp_socket,
        tls_acceptor,
        handlers,
        crate::messages::LARGE_PACKET_SIZE_LIMIT,
...
/// Process a TCP client connection while rejecting a declared frontend message
/// length before reading or decoding its body.
pub async fn process_socket_with_frontend_limit<H>(
    tcp_socket: TcpStream,
    tls_acceptor: Option<crate::tokio::TlsAcceptor>,
    handlers: H,
    max_frontend_message_length: usize,
...
where
    H: PgWireServerHandlers,
{
    if max_frontend_message_length < 4 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "frontend message length limit must be at least four bytes",
        ));
    }
    ...
    let socket = tokio::select! {
        ...
        socket = negotiate_tls_with_frontend_limit(
            tcp_socket,
            tls_acceptor,
            max_frontend_message_length,
        ) => {
