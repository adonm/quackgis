...
pub trait QueryHook: Send + Sync {
    /// Called before pgwire parses SQL. Hooks may return a replacement SQL
    /// string when a dialect-level compatibility rewrite is needed before
    /// sqlparser can build an AST. Returning `None` keeps the input unchanged.
    fn rewrite_sql(&self, _sql: &str) -> Option<String> {
        None
    }
