...
| C0 PostgreSQL compatibility | basic psql/psycopg/OGR smokes pass; `pg18-column-core-v1` and PostgreSQL 18.4 oracle frozen; DuckDB 1.5.4 multi-process identity gate proves DuckLake table UUID/ID and column field ID survive rename/reopen and name reuse gets new identity | capture pinned copied-data psql/OGR/QGIS traces; select/upstream the complete public DuckLake identity extraction boundary; implement transactional PostgreSQL OID/attribute mapping, core catalogs, then role/session semantics |
