...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 enforcement, relational role/membership catalogs, durable owner projection, bounded privilege inquiry, and role-aware schema/table/column/grant information-schema views consume the same role decisions; actual pgwire proves writer/ungranted/SELECT-only inquiry, discovery, and execution agreement | implement traced constraints/indexes/defaults/comments/spatial work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
