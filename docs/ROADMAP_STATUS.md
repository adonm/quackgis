...
| C0 PostgreSQL compatibility | basic scalar smokes plus core profile/oracle; exact OGR 3.11.5, psql 18.3 `\d+`, and QGIS 3.44.11 PostgreSQL/PostGIS traces frozen; exact-1.5.4 development lane now projects its transactional registry through `pg_namespace`/`pg_class`/`pg_attribute`/composite `pg_type`, direct-column RowDescription origins, and prepared-read epoch invalidation with rename/reopen/tombstone/drop-recreate/non-public-schema pgwire evidence | obtain upstream acceptance and a signed official identity contract; implement `reg*`, traced constraints/indexes/defaults/comments/spatial/privilege catalogs, broader provenance, and privilege-aware information schema; capture RBAC/OpenAPI references with C4/C5 |
