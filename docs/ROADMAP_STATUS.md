...
| DuckLake alignment | SQLite/local is the spec-oriented single-catalog path but is not yet drop-in DuckDB-writable; `docs/DUCKLAKE_ALIGNMENT.md` records that the current PostgreSQL multicatalog backend is library-specific/non-spec; `docs/DUCKDB_ADBC_EVALUATION.md` names DuckDB's official DuckLake extension as the preferred reference-reader gate before any DuckDB/ADBC writer pivot; `just duckdb-engine-probe` passed against DuckDB v1.5.2 for `spatial` + `ducklake` extension loading/WKB smoke, while the first DuckDB `ducklake:sqlite:` reference attach to a QuackGIS preview catalog failed on missing allocator metadata; `just duckdb-authority-probe` passed the DuckDB-authored official DuckLake vertical slice for create/insert/update/delete/reopen/WKB/snapshot metadata; `just duckdb-reference-evidence-check` now validates both non-standard legacy packets and the positive `duckdb-local-ducklake` reference-readable packet | route minimal pgwire/COPY through DuckDB-authored storage, then promote to managed PostgreSQL/S3 |
...
| Security/RBAC | SCRAM/read-only/TLS docs and tests; a structural read-only allowlist denies DDL/DML/maintenance/indeterminate statements before catalog refresh with SQLSTATE `42501`; write-capable service identities can be restricted with normalized DuckLake table allowlists and matching explicit-user privilege metadata; read-capable service identities can now use the same normalized table allowlist, with non-allowlisted DuckLake reads plus unfiltered `pg_catalog`/`information_schema`/DuckLake metadata UDTFs denied fail-closed and counted; bounded error logs, denial metrics, and redacted `quackgis_audit` events for auth failures, read/write denials, and compaction maintenance are covered; `docs/SECURITY_RBAC.md` defines the RBAC target | execute external secret/TLS failure drills, replace fail-closed metadata denial with trace-driven filtered metadata views where required, expand audit coverage to snapshot/restore/retention/admin changes, and split future administrative permissions |
