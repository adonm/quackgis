...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 enforcement/discovery plus packaged role-aware REST prove PostgreSQL types, role/legacy filtering, durable schema/security epochs, exact signed-runtime fallback, OpenAPI, and execution agreement; pinned psycopg copied-data and OGR SQL-result package slices pass | primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain blocked by DuckLake. Retried psql describe and OGR direct discovery still fail at `pg_class`/`pg_proc`; QGIS 3.44.11 additionally reports durable OID inquiry and `pg_is_in_recovery`/transaction-state gaps. Obtain the official identity bundle before copied-data psql/QGIS and OGR direct discovery/COPY/no-FID closure |
| H0 role-aware HTTP | packaged two-replica implementation complete at K0 | bounded HS256 JWT validation drives an exact credential-bound `authenticator` lease, transaction-local role/claims, grant-filtered per-role caches, role-specific OpenAPI, exact signed-runtime cache validation, two ready endpoints, one-Pod failover, core reconnect, and old authenticator/JWT denial. Signed shared epochs, public HTTP TLS/rate limits, multi-key overlap, full PostgREST, and RLS remain open |
...
- The direct-pgwire REST preview validates HS256 signatures against one
  ...
  shared epochs; packaged signed runtime uses the exact digest fallback. Two
