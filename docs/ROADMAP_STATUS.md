...
| N0 pinned native bundle | next active dependency; design accepted, implementation not started | replace the DuckLake-only source builder plus separately accepted DuckDB/Spatial artifacts with one manifest, one central DuckDB build, exact compatible DuckLake/Spatial sources, ordered patch queues, additive QuackGIS extension, upstream/QuackGIS tests, immutable trust policy, SBOM, and upgrade/rollback matrix; current commands remain authoritative until equivalent N0 gates pass |
