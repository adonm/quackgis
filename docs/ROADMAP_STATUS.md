...
| I0 early iroh transport | active; local-direct DuckDB parity implemented | shared `control/1`/`edge/1`, config-backed registered credential/worker, bounded signed leases and nonce replay cache, challenged transport-bound key proof, typed pgwire/cancellation streams, mandatory `none`, bounded loopback tiny client, role-bound startup, nested-TLS denial, and backend credential-challenge denial pass real local iroh endpoints. One differential oracle now gives direct TCP and local iroh identical result/type/error, parameter, portal, spatial, transaction/disconnect, COPY success/failure atomicity, cancellation/quarantine, concurrent-session, and fresh-reconnect outcomes; next run public-default/custom-relay correctness and resource profiles |
...
- The I0 binaries load owner-only key files and validate omitted-public versus
  ...
  The native direct gate differentially covers result values/types, stable errors,
  scalar/typed/spatial queries, portals, commit/rollback/disconnect, successful and
  malformed COPY atomicity, cancellation/quarantine, concurrent sessions, and
  fresh reconnect. Public/custom relay paths, resource budgets, packaged
  local-boundary protection, and role-catalog preauthentication remain open.
