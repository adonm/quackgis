...
| N0 pinned native bundle | active; current central candidate is upstream-tested and unaccepted | authority `c6d896e…` builds exact CLI `4eea46…`, library `217438…`, DuckLake `012cf9…`, and Spatial `55496d…` from the hardened cache-disabled `DONT_LINK` graph. The gate proves both extensions start absent, loads exact candidate files, verifies source revisions, and passes DuckLake functions (143 assertions/3 cases), the separately enforced patch regression (59/1, overlapping that function group), and complete Spatial SQL (1,607/130). Exact tool/source/runner/output pins and atomic bootstrap/metadata/runtime publication pass. Release remains blocked: Spatial bytes differed across three otherwise identical clean builds (`2430bfde…`, `1158cf4a…`, `55496d8d…`), DuckDB/patch/Spatial licenses are incomplete, the pristine-vs-patched differential is open, and all seven QuackGIS groups must run on candidate bytes before lifecycle/upgrade/rollback acceptance |
...
| Runtime supply chain | `just native-bundle-check`, `just native-upstream-check`, `just native-bundle-prepare`, `just native-bundle-configure`, `just native-bundle-build`, `just native-bundle-candidate-test`, `just native-bundle-metadata`, `just duckdb-pinned-ducklake-test`, and `just duckdb-runtime-offline-smoke` | current authority/source/result-tree/live-upstream, merged-vcpkg (`1139b3…`), exact executable, build, custom-path load, complete declared upstream, patch, metadata, bootstrap, and current selected-runtime gates pass. Candidate plan state is `upstream-tested-unaccepted`; it records exact four-artifact/test-runner/vcpkg hashes plus all seven pending QuackGIS groups. Source/toolchain reuse rejects hidden/ignored/staged/symlinked/cached/caller-controlled inputs and publication is atomic. Current legacy runtime evidence remains selected; central Spatial nondeterminism, license conclusions, candidate product/package tests, reopen/recovery/upgrade/rollback remain open |
