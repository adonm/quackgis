...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 now has bounded immutable role/owner/grant configuration, graph validation, per-connection session/effective identity, set-option role assumption, local role lifecycle, prepared invalidation, and bounded transaction-local request context through actual pgwire | add the focused role/context cancellation-quarantine oracle, then C5 role catalogs, common privileges, constraints/indexes/defaults/comments/spatial/information-schema work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
...
| Auth/policy | real CLI SCRAM, table allowlist, role-session, and request-context cases in pgwire workflow | bounded immutable role graph; original-login set-option traversal; name-typed session/effective identity; session/local role switching, reset, transaction cleanup, connection isolation, and prepared invalidation. Exact 16 KiB transaction-local `request.jwt.claims` set/get passes assignment/NULL/oversize/stale/commit/failed-rollback cases. Legacy allowlists still authorize objects; common grants remain open |
...
| M3 focused compatibility | C3 foundation complete; C4 implementation substantially complete; C5 next | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user class/attribute/composite-type catalogs, registered-object resolution, RowDescription origins, and commit-aware schema epochs are verified in the exact-1.5.4 gated lane. Immutable role graph/session identity, role switching, local cleanup, bounded request context, and prepared invalidation now pass actual pgwire. Remaining: focused role/context cancellation-quarantine evidence; upstream identity bundle; common privileges/role catalogs; traced constraint/index/default/comment/spatial catalogs and privilege-aware information schema; REST epoch consumption; richer provenance; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
