...
| K0 minimal Kind topology | complete | clean source `e6aef28` on rootless Podman runs runtime `localhost/quackgis-duckdb-runtime@sha256:2c2f1e1d99162f01bfdcd45dd59a76ac70f74351bc7eb0653da8a6f90ddbe3a3`. One core StatefulSet Pod has a 4 GiB container limit and loopback-only role-catalog edge preauthentication; bootstrap maps distinct credentials to `postgres`, `authenticator`, and `migration_operator`. Separate ordinary and migration mutual-TLS ingresses, pinned psql/psycopg/OGR, direct/plaintext/certificate denials, two REST Pods, role/failover, and QGIS 3.44.11 pass. The migration lane additionally stages/promotes, restarts the core in 3.906 s, and reruns all four named clients over promoted data without adding PostGIS or QGIS to normal startup |
| N0 pinned native bundle | active; authority, clean-source preparation, consumer unification, and metadata packaging complete | `native/bundle.json` exact-pins the common DuckDB 1.5.4 core, compatible DuckLake/Spatial full commits and trees, selected artifact digests, shared toolchain, test groups, and package outputs. Closed ordered queues record base/result trees and the DuckLake identity patch. `just native-bundle-check` rejects authority drift; `just native-bundle-prepare` fetches only exact commits, applies patches in order, verifies resulting Git trees, leaves extension DuckDB submodules uninitialized, and fail-closed reuses recognized clean output. All native consumers use the one authority and runtime manifests carry path-free bundle/source/patch/toolchain identity. `just native-bundle-metadata` deterministically emits SPDX 2.3 plus a native license inventory; runtime assembly hashes/packages both, while exact Spatial transitive versions/licenses/notices remain explicitly release-blocking. Next execute the central build, complete those license materials, prove upstream adoption/deletion review, and run the lifecycle/upgrade matrix |
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. The packaged runtime executes all 12 captured psql `\d+` stages with exact typed empty optional-feature probes, OGR SQL-result/direct-discovery Point/NULL reads plus predeclared-target COPY, and QGIS's copied-data query-layer binary/filter/extent/viewport-identify/render workflow. Durable schema/security epochs and role-aware REST/OpenAPI pass in the same matrix | generic SRID-0 metadata and truthfully empty key/index projections remain the supported floor; S0 and Q0 now own authoritative expansion after N0 rather than ad hoc compatibility rows |
| S0 authoritative CRS | queued behind N0 | qualify released CRS-aware native geometry, official coordinate-system catalogs, DuckLake lifecycle persistence, PostgreSQL SRID mapping, migration, and OGR/QGIS behavior before adding a DuckLake type-fidelity patch; do not fork Spatial for catalog presentation |
| Q0 validated keys | queued behind N0 and an explicit client-scope decision | decide whether direct-table QGIS/OGR creation are Local 1.0 requirements; if yes, prove durable declarations plus NOT NULL/uniqueness enforcement across every supported write/recovery/upgrade path before publishing key/index rows |
...
| G0 offline PostGIS migration | active; staged packaged lifecycle passes, no general release migration claim | direct evidence proves repeatable-read concurrent-write exclusion, rollback, exact retry checksums, report-bound cleanup/promotion, and pre-target key/report rejection. Exact inventoried source roles and maintained table/column grants map to independently provisioned immutable target policy without role DDL or credentials. Clean packaged source `e6aef28` binds the migrator/artifact/source/image identities, stages and atomically promotes 10,004 rows through the dedicated mTLS/iroh `migration_operator` path, restarts K0, and passes psql/psycopg/OGR/QGIS against promoted data; the ordinary client certificate is denied. Atomic bounded progress now covers preflight, per-table COPY/verification, commit state, rollback, and terminal decisions; Point reports now include family/SRID/dimensions, structural WKB validity, empty/invalid counts, and finite extents; add broader semantic validity and supported semantics |
...
| U0 upstream adoption | folded into N0 candidate selection where native | conditional deletion/adoption still covers scheduled DuckDB releases, PEG, Quack, async I/O, extension APIs, native spatial/pruning, and DuckLake protected snapshots/RBAC/materialized views; N0 now owns exact native candidate resolution and executable bundle matrices |
...
| Offline PostGIS snapshot | `just postgis-migration-smoke` | digest-pinned PostgreSQL 18.4/PostGIS 3.6.4 and an actual DuckDB 1.5.4 QuackGIS process inventory two schemas and stream 100,002 scalar rows plus two Point/NULL rows (1,249,051 wire bytes). A concurrent source commit raises the source to 100,003 but is excluded from the held snapshot; fresh-session canonical row/column checksums pass. Invalid date rolls back every table and a primary key rejects before target access. The lifecycle oracle then rejects a wrong report digest before target access, cleans only exact staged-report targets with no release visibility, retries into a fresh stage with identical checksums, promotes only an exact verification-report pair, and leaves 100,003 release rows with zero staging residue |
| Packaged PostGIS migration | `just kind-postgis-migration-gate` | clean source `e6aef28` packages `quackgis-migrate` in runtime `localhost/quackgis-duckdb-runtime@sha256:2c2f1e1d99162f01bfdcd45dd59a76ac70f74351bc7eb0653da8a6f90ddbe3a3`. The report records `source_dirty=false`, source SHA, executable/artifact digests, and target image digest. A digest-pinned PostgreSQL 18.4/PostGIS 3.6.4 sidecar supplies two schemas; the Job moves 10,002 scalar rows plus two Point/NULL rows into fresh `g0stage__*` tables through the credential-bound mTLS/iroh `migration_operator`, verifies exact report digests, and atomically promotes with zero stage residue. A 3.906 s core restart retains the release; pinned psql 18.3, psycopg 3.2.13, OGR 3.11.5, and QGIS 3.44.11 match promoted counts/scalars/Point/NULL/filter/extent/render. Ordinary-client-certificate, direct-worker, plaintext, and certificate-free access fail. The source sidecar is gate-only and absent from normal K0 startup |
...
| Spatial gaps | `docs/DUCKDB_SPATIAL_GAP_LEDGER.md` | 8 Rust/catalog-edge gaps and 5 extension candidates have ledger-pinned `0A000` simple/extended pgwire behavior; semantics remain unsupported |
...
| Runtime supply chain | `just native-bundle-check`, `just native-bundle-prepare`, `just native-bundle-metadata`, `just ducklake-pinned-source-check`, `just duckdb-pinned-ducklake-test`, and `just duckdb-runtime-offline-smoke` | the N0 candidate authority exact-pins full common DuckDB/DuckLake/Spatial commits and trees, the common core target, selected artifacts/toolchain/tests/outputs, and ordered patch queues with resulting-tree identity. Real preparation fetches only those commits, leaves embedded core submodules absent, applies the DuckLake patch in order, verifies exact resulting trees, and validates safe reuse. All native bootstrap/build/static/package consumers use this authority; runtime assembly records its path-free digest, source/base/result trees, patches, toolchain, and build options. Deterministic SPDX 2.3 and license inventory outputs are hashed into the runtime manifest and copied into the image; the inventory remains truthfully incomplete/local-only with each Spatial transitive dependency release-blocking. Existing clean source `e5f57b3` produced local OCI digest `sha256:52ace7276f7cdf36e0b6fb3c77110bc4434b9aafed7c786484933d6abd6e4ec2`; the pre-N0 artifacts still prove immutable load-only startup. Source `e6aef28` includes `quackgis-migrate` and its Kind-bound image evidence. Central build, complete license materials, and current-upstream adoption review remain open |
...
- The offline migrator requires an exact PostgreSQL/PostGIS version pin, opens one
  ...
  CA, verifies 10,004 rows, and rejects the ordinary K0 client certificate. Fresh
  staging, exact-report verification and cleanup, atomic promotion, clean runtime
  manifest/source/image identity, K0 restart, and promoted-data
  psql/psycopg/OGR/QGIS reads pass. Exact inventoried owner/grantee roles and
  maintained table/column grants map explicitly to independently provisioned
  immutable target policy; credentials and role DDL remain excluded. Atomic
  bounded progress covers preflight, per-table COPY/verification, commit state,
  rollback, and final decisions without becoming a resume/promotion token.
  Maintained Point reports include family/SRID/dimensions, structural WKB
  validity, empty/invalid counts, and finite extents. Broader semantic validity
  and key/CRS/geometry semantics remain open.
...
| G0 offline PostGIS migration | active; staged lifecycle and named-client slice complete | exact-version preflight classifies selected-schema tables/columns/constraints/views/sequences/indexes/extensions/functions/triggers/roles/grants; accepted release scalars, literal defaults, comments, and 2D SRID-0 Point/NULL stream from one repeatable-read snapshot. Exact source role/grant mappings are audit-visible and bind only to independently provisioned target policy. Canonical checksums pass before commit and after reconnect; concurrent source writes are excluded, invalid input rolls back every target, and key/report mismatches reject before target access. Fresh staging, exact report-bound verify/cleanup/promotion, runtime manifest/source/image identity, dedicated credential/role/CA ingress, ordinary-certificate denial, K0 restart, and promoted-data psql/psycopg/OGR/QGIS pass. Atomic bounded progress checkpoints cover preflight, per-table COPY/verification, commit state, rollback, and terminal decisions. Point reports include family/SRID/dimensions, structural WKB validity, empty/invalid counts, and finite extents. Remaining: broader semantic validity, interruption/resource qualification at larger scale, and expansion beyond the initial fail-closed type/geometry set. |
| M5 Local 1.0 | active; K0 package boundary complete | immutable load-only runtime smoke, local readiness/drain/compaction, runtime-bound actual-process checksum backup/exact-path checkpoint recovery, reduced mixed read/COPY/mutation/cancel/compaction/restart workload, host TLS/SCRAM and direct REST rotation, and packaged direct iroh exist. Rootless-Podman Kind uses node-local digest aliases, retained storage, one ordered core Pod, mutual-TLS `postgres` ingress, and two load-balanced authenticator REST Pods with no database password. Psql full describe, psycopg COPY/reconnect, OGR SQL-result/direct readback and predeclared-target COPY/reopen, optional QGIS query-layer filtering/viewport/render, role-aware REST, one-Pod failover, and core reconnect pass; direct/plaintext/certificate-free access fails; a 3.935 s core replacement reconnects all gates; packaged mTLS/edge/authenticator/JWT replacement denies old credentials. Host I0 profiles remain authoritative for performance. N0 is the immediate unblocked release dependency; scheduled/unreleased candidates remain evidence inputs, not a reason to defer its tooling. Remaining operations include resource/hosted-relay reruns, catalog/control lifecycle, published artifacts, online/relocated disaster recovery, cross-version upgrade, and the 24-hour soak. S0/Q0 are separately ordered compatibility decisions. |
