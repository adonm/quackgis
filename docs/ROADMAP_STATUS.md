...
| C0 PostgreSQL compatibility | basic psql/psycopg/OGR smokes pass; core profile/oracle frozen; exact OGR 3.11.5 copied-point and psql 18.3 `\d+` traces captured against digest-pinned PostgreSQL 18.4/PostGIS; durable DuckLake identity gate and relational spatial type views pass | capture headless QGIS 3.44 and later RBAC/OpenAPI traces; select/upstream complete public DuckLake identity extraction; implement transactional PostgreSQL OID/attribute mapping and traced user-object/spatial catalogs |
...
| PostgreSQL 18 target profile | `just project-contract-check` | machine-readable `pg18-column-core-v1` validates required relation/query/result-type shape against a digest-pinned PostgreSQL 18.4 oracle. Credential-free OGR 3.11.5 copied-point (21 queries) and psql 18.3 `\d+` (12 queries) traces pin client and PostgreSQL/PostGIS images; QGIS/RBAC traces stay explicit. Target traces are not QuackGIS implementation evidence |
...
| M3 focused compatibility | active foundation | maintained SET/SHOW, AST `public` mapping, quoted COPY, `pg18-column-core-v1`, PostgreSQL 18.4 oracle, durable DuckLake identity, exact psql 18.3 `\d+`, and exact OGR 3.11.5 copied-point traces are verified. Spatial type interception is replaced by relational namespace/type/range views with PostgreSQL 18 wire types and geometry/geography transport identity. Kind scalar smokes pass for psql, psycopg, and OGR. Remaining: QGIS/RBAC traces; public complete DuckLake identity extraction; transactional OID/attribute registry and epoch; built-in/user-object class/attribute/database/constraint/spatial catalogs, implicit lookup and RowDescription origins; roles/privileges/information schema; copied-data clients; then role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
