...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane: client-neutral foundation descriptions, transactional registry-backed class/attribute/composite-type catalogs, search-path `reg*`/`format_type`, direct/join/wildcard RowDescription origins, strict errors, and epoch invalidation pass rename/reopen/tombstone/drop-recreate/non-public-schema pgwire evidence; named PostgreSQL/PostGIS traces remain frozen targets | obtain upstream acceptance and a signed official identity contract; proceed to C4 roles/session and C5 constraints/indexes/defaults/comments/spatial/privilege/information-schema work; then run copied-data named clients and role-aware REST |
...
| M3 focused compatibility | C3 foundation complete; C4/C5 active next | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, session bootstrap, user class/attribute/composite-type catalogs, registered-object resolution, RowDescription origins, and commit-aware schema epochs are verified in the exact-1.5.4 gated lane. Remaining: upstream merge/signed-bundle release evidence; roles/session/privileges; traced constraint/index/default/comment/spatial catalogs and privilege-aware information schema; REST epoch consumption; richer provenance where named clients require it; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
