...
| N0 pinned native bundle | active; common authority slice complete | `native/bundle.json` now exact-pins the common DuckDB 1.5.4 core, compatible DuckLake/Spatial full commits and trees, selected artifact digests, shared toolchain, test groups, and package outputs. Closed ordered patch queues record base/result trees and the DuckLake identity patch; `just native-bundle-check` rejects schema, core-target, path, or digest drift in CI. Next prepare all three clean sources for the declared central build, migrate artifact consumers off the duplicated DuckLake pin, emit SBOM/license provenance, and run the build/lifecycle/upgrade matrix |
