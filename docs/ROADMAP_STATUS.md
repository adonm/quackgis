...
| P0 M4 host profiles | active foundation; conservative single-table injection passes | extend the geometry matrix and scan-plan oracle, then run 10M twice before 100M |
...
| M3 focused compatibility | active foundation | core profile/oracle, durable DuckLake identity, and exact psql/OGR/QGIS traces are verified. Relational views provide PostgreSQL 18 built-in/spatial/logical-database identity, maintained database/schema/search-path discovery, and fail-closed routing/provenance; the exact QGIS four-`SET` bootstrap passes through pgwire. Kind scalar smokes pass. The public DuckLake identity-function boundary is selected and prototyped. Remaining: upstream merge and pinned-runtime identity evidence; transactional OID/attribute registry/epoch; user-object class/attribute/constraint/spatial/privilege catalogs, richer provenance, `reg*` and RowDescription origins; roles/information schema/RBAC traces; actual copied-data clients; role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
| M4 analytical performance | active foundation | fail-closed COPY bbox maintenance, atomic numbered-bound/NULL geometry UPDATE refresh, direct INSERT/arbitrary geometry/reserved UPDATE rejection, ordinary-column UPDATE preservation, and ordinary file compaction are implemented. Conservative one-table AST injection now handles bounded literal envelope/text and numbered-bound WKB probes, retains exact recheck, and passes hole/boundary/NULL/reopen/plan evidence. Remaining: broader geometry mutation/spatial-compaction policy, invalid/empty geometry matrix, scan-byte plans, and current 10M then 100M profiles. |
