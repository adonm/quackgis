...
| N0 pinned native bundle | active; authority, preparation, consumer unification, metadata packaging, and upstream review complete | the common manifest/queues pin exact compatible sources, base/result trees, selected artifacts, toolchain, tests, and outputs. Preparation and all native consumers use that authority; deterministic SPDX/license outputs are packaged with unresolved Spatial details release-blocking. `native/upstream-review.json` plus live `just native-upstream-check` records the latest DuckDB release/current extension refs and one adopt/retain/delete decision per capability/patch. Current review confirms DuckDB 1.5.4 and the selected DuckLake 1.5 tip are current; selected Spatial matches released core while its newer branch targets unreleased core. It requires S0 to adopt existing upstream CRS, retains the identity patch only for a confirmed public-API gap, and forces M4 reevaluation/deletion against newer upstream R-tree work. Next execute the central build, complete license materials, and run lifecycle/upgrade matrices |
