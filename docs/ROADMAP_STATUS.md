...
| P0 M4 host profiles | active; scan/byte/compaction oracle and two 10M point runs pass | add representative non-point shapes plus query-latency/resource budgets, decide the first-release representation, then introduce 100M |
...
| Spatial pruning | `just duckdb-adbc-storage-test`, `just duckdb-pgwire-workflow-test`, and `just duckdb-spatial-scan-profile` | one-table maintained-layout `ST_Intersects` adds planner-visible four-axis candidates while retaining the exact predicate. Clean 100k, 1M, and two 10M point profiles on source `ba658ba` compare 25 ordered DuckLake files per layout. Every maintained WKB/bbox and native `GEOMETRY` query returns the same 100 exact pgwire hits. The 10M runs scan one of 144/146 bbox and 100 native row groups; conservative compressed-byte upper bounds are 1.176%/0.939% and 1.272%. Both layouts compact 25 files to one without result changes. Non-point and broad workload claims remain open |
...
- COPY validates and maintains the explicit four-column bbox layout in DuckDB SQL;
  ...
  profiler reports N scans. Clean 100k, 1M, and two 10M point runs pass exact
  pgwire, 5% scan-byte, 20x row-group, and 2x compaction gates; both 10M layouts
  compact from 25 files to one without result changes. Native geometry is about
  51% smaller for this point fixture, but automatic DDL, broader geometry shapes,
  mutation policy, query latency/resources, and 100M evidence remain open.
...
| M4 analytical performance | active; two 10M point scan-oracle runs pass | fail-closed bbox maintenance, exact recheck, edge-case correctness, compressed scan-byte accounting, and spatial fragmented-file compaction evidence are implemented. Clean 100k/1M and consecutive 10M maintained-bbox/native-geometry profiles return equal exact results and stay below 5% compressed bytes with at least 20x row-group reduction; 25 files compact to one without result changes. Remaining: representative non-point shapes, broader mutation policy, query latency/RSS/spill budgets, first-release representation decision, then two 100M profiles. |
