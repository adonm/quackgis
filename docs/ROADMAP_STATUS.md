...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 common enforcement combines schema USAGE, table ownership, direct/inherited/PUBLIC grants, and the legacy outer ceiling; relational `pg_roles`/`pg_auth_members` expose stable resolving role/edge identities and options without credentials through actual pgwire | add privilege inquiry from the same engine, then constraints/indexes/defaults/comments/spatial/information-schema work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
...
| Auth/policy | real CLI SCRAM, table allowlist, role-session, request-context, effective-role grant/denial, and relational role/membership catalog cases in pgwire workflow | bounded immutable role graph; name-typed identity; role/context lifecycle; common privilege enforcement; stable role/edge OIDs and resolving PostgreSQL 18 membership options with NULL credentials. Inquiry remains open |
...
| M3 focused compatibility | C3/C4 complete; C5 active | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user catalogs/origins/epochs, immutable role sessions/context, cancellation isolation, common privilege enforcement, and role/membership catalogs pass actual pgwire. Remaining: upstream identity bundle; privilege inquiry; traced constraint/index/default/comment/spatial catalogs and privilege-aware information schema; REST epoch consumption; richer provenance; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
