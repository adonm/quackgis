...
| C0 PostgreSQL compatibility | basic scalar smokes plus core profile/oracle; exact OGR 3.11.5 copied-point, psql 18.3 `\d+`, and successful offscreen QGIS 3.44.11 open/inspect/count/extent/read traces captured against digest-pinned PostgreSQL 18.4/PostGIS; durable identity and relational spatial type views pass; upstream public DuckLake identity-function design selected and prototyped | obtain upstream acceptance and pin the public identity contract; implement transactional PostgreSQL OID/attribute mapping and traced user-object/spatial/privilege catalogs; capture RBAC/OpenAPI references with C4/C5 |
...
| Catalog contract | `just duckdb-catalog-contract-test` | explicit/implicit namespace/type/range/collation/owner-role mapping passes with 24 exact PostgreSQL 18 built-ins plus PostGIS-shaped spatial scalar/array rows. Full-row oracle equality, reciprocal references, inferred/explicit OID parameters, wire types, alias safety, and spatial transport pass. All unimplemented/private routes plus wildcard/nested/set/derived/implicit-join/CTE/cross-database shapes fail closed; `TABLE` is rejected before authorization. User-object catalogs/origins remain open |
...
| Catalog identity feasibility | `just duckdb-catalog-identity-test` | independent DuckDB 1.5.4 processes prove table ID/UUID stability over table rename/reopen, column field-ID stability over column rename, exact current names/data, and new identity after drop/recreate. Durable DuckLake IDs are registry keys, not PostgreSQL OIDs. The lower-maintenance boundary is an upstream committed-snapshot `ducklake_column_info` function; its main-branch prototype passes, but merge and pinned-runtime evidence remain open |
...
- Broad `pg_catalog`/`information_schema`, spatial discovery, SRID, dimension,
  ...
  process-local namespace/type/range/collation/owner views resolve the frozen core
  and QGIS-required built-ins, all referenced arrays/collations, and both spatial
  sentinels through explicit/implicit structural mapping. PostgreSQL 18 lookup
  types and spatial RowDescription/text/binary/NULL behavior pass through pgwire.
...
| M3 focused compatibility | active foundation | core profile/oracle, durable DuckLake identity, and exact psql/OGR/QGIS traces are verified. Relational views provide PostgreSQL 18 built-in/spatial identity and fail-closed routing/provenance; the exact QGIS four-`SET` bootstrap passes through pgwire. Kind scalar smokes pass. The public DuckLake identity-function boundary is selected and prototyped. Remaining: upstream merge and pinned-runtime identity evidence; transactional OID/attribute registry/epoch; user-object class/attribute/database/constraint/spatial/privilege catalogs, richer provenance, `reg*` and RowDescription origins; roles/information schema/RBAC traces; actual copied-data clients; role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
