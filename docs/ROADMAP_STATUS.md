...
| N0 pinned native bundle | active; foundations and central dependency configuration complete | the common authority, exact-source preparation, consumer unification, deterministic SPDX/license metadata, and live upstream adopt/retain/delete review pass. `just native-bundle-configure` now loads prepared DuckLake/Spatial into one DuckDB 1.5.4 CMake graph with both test suites, pinned GCC/CMake/Ninja, and one normalized ten-entry merged vcpkg graph; its exact digest and path-free plan pass. Current upstream review directs S0 to adopt existing CRS, retains the identity patch only for a confirmed public-API gap, and forces M4 reevaluation against newer R-tree work. Next run/accept central artifact builds, complete Spatial license materials, and execute upstream/QuackGIS/lifecycle/upgrade/rollback matrices |
...
| Runtime supply chain | `just native-bundle-check`, `just native-upstream-check`, `just native-bundle-prepare`, `just native-bundle-configure`, `just native-bundle-metadata`, `just ducklake-pinned-source-check`, `just duckdb-pinned-ducklake-test`, and `just duckdb-runtime-offline-smoke` | exact common authority/source/result-tree and live upstream checks pass. The central configure gate uses one DuckDB 1.5.4 checkout plus prepared DuckLake/Spatial, loads both upstream tests, verifies pinned GCC 16.1.1/CMake 4.3.3/Ninja 1.13.2, and emits a path-normalized ten-dependency merged vcpkg graph with digest `1139b3f4e66cb1cbb2233dab2b0c9641ded7dc548b502b36edc4f8bc58e286e4`. All native consumers use the authority; runtime manifests bind its linked digest and deterministic SPDX/license outputs. The current legacy artifacts still prove immutable load-only startup, but central candidate compilation/acceptance, complete Spatial licenses, and lifecycle matrices remain open |
