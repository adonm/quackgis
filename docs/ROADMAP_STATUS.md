...
| Runtime supply chain | `just ducklake-pinned-source-check`, `just duckdb-pinned-ducklake-test`, and `just duckdb-runtime-offline-smoke` | clean source `d4322df` produced local OCI digest `sha256:603204c4c23ae6cf473a24d0cb1318a33cfeaa53dd4cc74e55b4cbae130bf191`; tracked DuckLake base/patch/tool/artifact pins, exact compile-time/config/file digest agreement, immutable image path, signed Spatial plus the pinned read-only DuckLake extension, no online install, network-disabled CLI load, all binary version probes, and server startup pass |
