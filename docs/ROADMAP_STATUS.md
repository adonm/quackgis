...
| C0 PostgreSQL compatibility | basic psql/psycopg/OGR smokes pass; core profile/oracle frozen; exact OGR 3.11.5 copied-point discovery trace captured against digest-pinned PostgreSQL 18.4/PostGIS; durable DuckLake identity gate and relational spatial type views pass | capture copied-table psql and headless QGIS 3.44 traces; select/upstream complete public DuckLake identity extraction; implement transactional PostgreSQL OID/attribute mapping and the OGR-required user-object/spatial catalog surface |
...
| Catalog contract | `just duckdb-catalog-contract-test` | client-neutral fixture executes DuckDB-derived table/column metadata and structurally maps explicit namespace/type/range/owner-role references to protected private views. Provenance-bound wire hints provide OID parameters and seven PostgreSQL 18 lookup types; typed ordinary rows/values, unknown OIDs, public namespace/owner resolution, ordinary-alias safety, and both families' RowDescription/text/binary/NULL identity pass. User-object catalogs/origins and named clients remain open |
| PostgreSQL 18 target profile | `just project-contract-check` | machine-readable `pg18-column-core-v1` validates required relation/query/result-type shape against a digest-pinned PostgreSQL 18.4 oracle. A credential-free 21-query OGR 3.11.5 copied-point trace pins both client and PostgreSQL/PostGIS images; psql/QGIS/RBAC traces stay explicit. Target traces are not QuackGIS implementation evidence |
| Catalog identity feasibility | `just duckdb-catalog-identity-test` | independent DuckDB 1.5.4 processes prove table ID/UUID stability over table rename/reopen, column field-ID stability over column rename, exact current names/data, and new identity after drop/recreate. Durable DuckLake IDs are registry keys, not PostgreSQL OIDs; public schema/column identity extraction remains incomplete |
...
- Broad `pg_catalog`/`information_schema`, spatial discovery, SRID, dimension,
  extent, MVT, and general `ST_GeometryN` behavior remain incomplete. Relational
  process-local namespace/type/range views resolve only the maintained geometry
  and geography sentinel OIDs through structural explicit-reference mapping; both
  families' PostgreSQL 18 lookup types and RowDescription/text/binary/NULL
  behavior are proven through the current Rust pgwire client.
...
| M3 focused compatibility | active foundation | maintained SET/SHOW, AST `public` mapping, quoted COPY, `pg18-column-core-v1`, PostgreSQL 18.4 oracle, and durable DuckLake identity feasibility are verified. Exact spatial type interception is replaced by structural explicit-reference mapping to relational namespace/type/range views; parameter OID, seven PostgreSQL 18 result types, ordinary/unknown-OID queries, public namespace, and geometry/geography RowDescription/text/binary/NULL identity pass. Kind `SELECT 1` smokes pass with psql 18.3, psycopg 3.2.13, and OGR 3.11.5. Remaining: pinned copied-data traces; public complete DuckLake identity extraction; transactional OID/attribute registry and epoch; built-in/user-object class/attribute/database catalogs, implicit lookup and RowDescription origins; roles/privileges/information schema; copied-data clients; then role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
