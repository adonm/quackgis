...
| REST preview | `just rest-check` and `just rest-postgrest-smoke` | separate read-only sidecar pins `pg-rest-server` parser/schema inputs and proves HS256 validation/denial plus atomic key-file hot rotation with invalid-key readiness failure and old-key denial, one SCRAM authenticator, transaction-local role/claims cleanup, common-grant-filtered per-role PostgreSQL catalog/OpenAPI/direct denial, automatic live-column and stale-wide-role-cache revision invalidation, projection, typed filters, ordering, pagination, and escaped WKB through actual QuackGIS pgwire; shared monotonic epochs, authenticator/database rotation, full PostgREST parity, writes, RPC, embedding, GeoJSON, tiny-client routing, and packaged deployment remain open |
...
| TLS/credential rotation | `just duckdb-tls-rotation-profile`, `just kind-secret-rotation-gate`, and `just rest-postgrest-smoke` | actual-process TLS-required/SCRAM smoke verifies client hostname/trust, rejects plaintext and wrong trust, restarts with replacement certificate/password while preserving committed state, rejects old trust/password, and accepts a post-rotation write. Packaged Kind independently rotates its mutual-TLS and iroh keys, denies the old client certificate, and reconnects every gate. Direct REST atomically hot-rotates its HS256 file, accepts the new key, denies the old key, and fails readiness for invalid material; authenticator/database and packaged JWT rotation plus production revocation remain open |
