...
| C0 PostgreSQL compatibility | basic scalar smokes plus core profile/oracle; exact OGR 3.11.5 copied-point, psql 18.3 `\d+`, and successful offscreen QGIS 3.44.11 open/inspect/count/extent/read traces captured against digest-pinned PostgreSQL 18.4/PostGIS; durable identity and relational spatial type views pass | select/upstream complete public DuckLake identity extraction; implement transactional PostgreSQL OID/attribute mapping and traced user-object/spatial/privilege catalogs; capture RBAC/OpenAPI references with C4/C5 |
...
| M3 focused compatibility | active foundation | `pg18-column-core-v1`, PostgreSQL 18.4 oracle, durable DuckLake identity, psql 18.3 `\d+`, OGR 3.11.5 copied-point, and QGIS 3.44.11 open/inspect/count/extent/read traces are verified. Spatial type interception is replaced by relational views with PostgreSQL 18 wire identity. Kind scalar smokes pass for psql, psycopg, and OGR. Remaining: public complete DuckLake identity extraction; transactional OID/attribute registry and epoch; traced built-in/user-object/constraint/spatial/privilege catalogs, implicit lookup and RowDescription origins; roles/privileges/information schema and dependent RBAC traces; actual copied-data clients; role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
