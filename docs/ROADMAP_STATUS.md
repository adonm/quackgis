...
| M3 focused compatibility | active foundation | core profile/oracle, durable DuckLake identity, and exact psql/OGR/QGIS traces are verified. Relational views provide explicit/implicit lookup, 24 exact PostgreSQL 18 built-ins, PostGIS-shaped spatial scalar/arrays, complete references, wire/OID parameter identity, and fail-closed routing/projection boundaries. Kind scalar smokes pass. Remaining: public complete DuckLake identity extraction; transactional OID/attribute registry/epoch; user-object class/attribute/database/constraint/spatial/privilege catalogs, richer provenance, `reg*` and RowDescription origins; roles/information schema/RBAC traces; actual copied-data clients; role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
