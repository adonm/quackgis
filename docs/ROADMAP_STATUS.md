...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 enforcement, relational role/membership catalogs, durable owner projection, and bounded PostgreSQL 18 role/schema/table/column inquiry consume the same role decisions; actual pgwire proves writer/ungranted/SELECT-only inquiry and execution agreement | add privilege-aware information schema, then traced constraints/indexes/defaults/comments/spatial work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
...
| Auth/policy | real CLI SCRAM, table allowlist, role-session, request-context, effective-role grant/denial, relational role/membership catalogs, and role/schema/table/column inquiry cases in pgwire workflow | bounded immutable graph and identity lifecycle; common enforcement and inquiry decisions; stable role/edge OIDs and resolving PostgreSQL 18 options with NULL credentials. Name inquiry works in the official lane; OID/expression inquiry is durable-identity gated; information-schema/OpenAPI consistency remains open |
...
| M3 focused compatibility | C3/C4 complete; C5 active | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user catalogs/origins/epochs, immutable role sessions/context, cancellation isolation, common privilege enforcement, role/membership catalogs, owner projection, and bounded role/schema/table/column inquiry pass focused/actual pgwire evidence. Remaining: upstream identity bundle; privilege-aware information schema; traced constraint/index/default/comment/spatial catalogs; REST epoch consumption; richer provenance; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
