...
| G0 offline PostGIS migration | active; first actual-process snapshot slice passes, no release migration claim | source `77a99e9` runs digest-pinned PostgreSQL 18.4/PostGIS 3.6.4 inventory, one read-only repeatable-read snapshot, transactional target DDL/COPY, exact canonical verification, concurrent-write exclusion, rollback-with-zero-table-residue, and pre-target key rejection. Package the same run through the tiny client, bind target runtime digests, add isolated staging/promotion and role/grant mapping, then run named post-migration clients |
...
- The offline migrator requires an exact PostgreSQL/PostGIS version pin, opens one
  ...
  tables. This direct actual-process gate does not yet prove the packaged tiny
  client, automatic staging/promotion, target runtime digests, role/grant mapping,
  post-migration named clients, or broader key/CRS/geometry semantics.
...
| G0 offline PostGIS migration | active; first source-to-target slice complete | exact-version preflight classifies selected-schema tables/columns/constraints/views/sequences/indexes/extensions/functions/triggers/roles/grants; accepted release scalars, literal defaults, comments, and 2D SRID-0 Point/NULL data stream from one repeatable-read snapshot into one target transaction. Canonical checksums pass before commit and after reconnect; concurrent source writes are excluded, invalid input rolls back every target, and key semantics reject before target access. Remaining: packaged tiny-client and immutable-artifact evidence, runtime digest binding, fresh staging root and explicit promotion/cleanup, role/grant mappings, full verification dimensions, copied-data named clients, and supported expansion beyond the initial fail-closed type/geometry set. |
