...
| H0 role-aware HTTP | catalog-backed read-only direct-pgwire preview passes | schema discovery now consumes role-filtered PostgreSQL `information_schema` names and types under real SCRAM/grants while retaining the REST exposure ceiling; next add JWT role mapping, transaction-local role/context, role-aware OpenAPI, then route/package replicas through the tiny client |
...
| REST preview | `just rest-check` and `just rest-postgrest-smoke` | separate authenticated read-only sidecar pins `pg-rest-server` parser/schema inputs and proves SCRAM/common-grant-filtered PostgreSQL catalog discovery, projection, typed filters, ordering, pagination, OpenAPI, denial paths, and escaped WKB through actual QuackGIS pgwire; JWT role switching, full PostgREST parity, writes, RPC, embedding, GeoJSON, and packaged deployment remain open |
