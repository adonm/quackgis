...
| K0 minimal Kind topology | complete | clean source `e5f57b3` on rootless Podman runs runtime `localhost/quackgis-duckdb-runtime@sha256:52ace7276f7cdf36e0b6fb3c77110bc4434b9aafed7c786484933d6abd6e4ec2`. One core StatefulSet Pod has a 4 GiB container limit and uses loopback-only role-catalog edge preauthentication; bootstrap maps distinct proven credentials to `postgres` and `authenticator`. The mutual-TLS pgwire ingress plus pinned psql/psycopg/OGR gates pass, including full `\d+` and OGR direct discovery. Two REST Pods pass the existing role/failover matrix. The separate pinned QGIS 3.44.11 gate then passes exact copied-data query-layer fields/count/Point/NULL reads without adding its heavyweight image to normal startup |
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. The packaged runtime executes all 12 captured psql `\d+` stages with exact typed empty optional-feature probes, OGR SQL-result and direct-discovery Point/NULL reads, and QGIS's copied-data query-layer binary-cursor workflow. Durable schema/security epochs and role-aware REST/OpenAPI pass in the same matrix | primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain upstream gaps, not reasons to invent rows. Direct QGIS ordinary-table open remains gated on real key semantics; OGR-authored COPY and broader QGIS filtering/identify/render qualification remain open |
...
| Spatial | pgwire workflow + `tests/duckdb_spatial_compat.json` | 43 original PostGIS expressions: 31 native, 5 rewrites, 7 ledger macros; CRS/SRID readback and 2D extent execute without claiming unsupported PostGIS SRID assignment. Client-specific `ST_Zmflag`, `GeometryType`, and NDR `ST_AsBinary` normalize maintained WKB for pinned QGIS discovery/readback |
...
- Rootless-Podman Kind mTLS tiny-client smoke passes with psql 18.3. Pinned
  ...
  layer with an explicit `id` key and proves exact fields, two-row count, and
  Point/NULL binary-cursor values. Direct ordinary-table open remains unsupported
  without real primary/unique-key metadata. GeoServer, Martin, ORM, BI, and broader
  QGIS render/filter workflows remain unqualified.
