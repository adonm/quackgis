...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 now has bounded immutable role/owner/grant configuration, graph validation, per-connection session/effective identity, set-option role assumption, local role lifecycle, and prepared identity invalidation through actual pgwire | complete bounded transaction-local request context and cancellation/quarantine cleanup; then C5 role catalogs, common privileges, constraints/indexes/defaults/comments/spatial/information-schema work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
...
| Auth/policy | real CLI SCRAM, table allowlist, and role-session cases in pgwire workflow | bounded immutable role graph; original-login set-option traversal; name-typed session/effective identity; session/local role switching, reset, transaction cleanup, connection isolation, and prepared invalidation. Legacy allowlists still authorize objects; request context and common grants remain open |
...
| M3 focused compatibility | C3 foundation complete; C4 active; C5 next | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user class/attribute/composite-type catalogs, registered-object resolution, RowDescription origins, and commit-aware schema epochs are verified in the exact-1.5.4 gated lane. Immutable role graph/session identity, role switching, local cleanup, and prepared invalidation now pass actual pgwire. Remaining: request context and cancellation/quarantine cleanup; upstream identity bundle; common privileges/role catalogs; traced constraint/index/default/comment/spatial catalogs and privilege-aware information schema; REST epoch consumption; richer provenance; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
