...
| Catalog identity feasibility | `just duckdb-catalog-identity-test` plus opt-in `just duckdb-development-ducklake-test` | independent DuckDB 1.5.4 processes prove table ID/UUID stability over table rename/reopen, column field-ID stability over column rename, exact current names/data, and new identity after drop/recreate. Durable DuckLake IDs are registry keys, not PostgreSQL OIDs. The selected committed-snapshot `ducklake_column_info` function is now ported to exact DuckDB 1.5.4; its DuckLake tests and QuackGIS ADBC lifecycle gate pass under an isolated checksum-pinned unsigned development override. Upstream merge and signed-bundle evidence remain release gates |
...
| M3 focused compatibility | active foundation | core profile/oracle, durable DuckLake identity, and exact psql/OGR/QGIS traces are verified. Relational views provide PostgreSQL 18 built-in/spatial/logical-database identity, maintained database/schema/search-path discovery, and fail-closed routing/provenance; the exact QGIS four-`SET` bootstrap passes through pgwire. Kind scalar smokes pass. The public DuckLake identity-function boundary has an exact-1.5.4 checksum-pinned development lane with DuckLake and QuackGIS lifecycle evidence. Remaining: transactional OID/attribute registry/epoch; user-object class/attribute/constraint/spatial/privilege catalogs, richer provenance, `reg*` and RowDescription origins; upstream merge and signed-bundle release evidence; roles/information schema/RBAC traces; actual copied-data clients; role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
