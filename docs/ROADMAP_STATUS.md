...
| I0 early iroh transport | not implemented; active target | build one config-backed bootstrap/registered key/signed one-worker lease, tiny client, and complete-worker path first, then register direct TCP, iroh direct-path, public-relay-default, configured-relay, and adaptive-compression correctness/resource profiles before durable shared control, gossip, or storage work |
