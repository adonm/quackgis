...
| K0 minimal Kind topology | complete | rootless Podman runs one core StatefulSet Pod with a 4 GiB container limit and loopback-only role-catalog edge preauthentication; bootstrap maps distinct proven credentials to `postgres` and `authenticator`. The mutual-TLS pgwire ingress plus pinned psql/psycopg/OGR gates pass, including full `\d+`, OGR SQL-result/direct discovery, and OGR COPY into a predeclared table with exact reopen. Two REST Pods pass the role/failover matrix. The separate pinned QGIS 3.44.11 gate passes exact copied-data query-layer fields/count/Point/NULL reads without adding its heavyweight image to normal startup |
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. The packaged runtime executes all 12 captured psql `\d+` stages with exact typed empty optional-feature probes, OGR SQL-result/direct-discovery Point/NULL reads plus predeclared-target COPY, and QGIS's copied-data query-layer binary-cursor workflow. Durable schema/security epochs and role-aware REST/OpenAPI pass in the same matrix | primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain upstream gaps, not reasons to invent rows. Direct QGIS ordinary-table open remains gated on real key semantics; OGR-created tables and broader QGIS filtering/identify/render qualification remain open |
