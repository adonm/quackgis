...
| G0 offline PostGIS migration | active; staged packaged lifecycle passes, no general release migration claim | direct evidence proves repeatable-read concurrent-write exclusion, rollback, exact retry checksums, report-bound cleanup/promotion, and pre-target key/report rejection. Clean packaged source `e6aef28` binds the migrator/artifact/source/image identities, stages and atomically promotes 10,004 rows through the dedicated mTLS/iroh `migration_operator` path, restarts K0, and passes psql/psycopg/OGR/QGIS against promoted data; the ordinary client certificate is denied. Add explicit source role/grant mappings, progress checkpoints, richer spatial report dimensions, and broader supported semantics |
...
- The offline migrator requires an exact PostgreSQL/PostGIS version pin, opens one
  ...
  psql/psycopg/OGR/QGIS reads pass. Explicit source role/grant mapping, progress
...
| G0 offline PostGIS migration | active; staged lifecycle and named-client slice complete | exact-version preflight classifies selected-schema tables/columns/constraints/views/sequences/indexes/extensions/functions/triggers/roles/grants; accepted release scalars, literal defaults, comments, and 2D SRID-0 Point/NULL stream from one repeatable-read snapshot. Canonical checksums pass before commit and after reconnect; concurrent source writes are excluded, invalid input rolls back every target, and key/report mismatches reject before target access. Fresh staging, exact report-bound verify/cleanup/promotion, runtime manifest/source/image identity, dedicated credential/role/CA ingress, ordinary-certificate denial, K0 restart, and promoted-data psql/psycopg/OGR/QGIS pass. Remaining: explicit source role/grant mappings, bounded progress checkpoints, richer spatial report dimensions, interruption/resource qualification at larger scale, and expansion beyond the initial fail-closed type/geometry set. |
