...
| P0 M4 host profiles | active; two 10M mixed-shape scan/resource runs pass | Local 1.0 retains maintained WKB/bbox storage; introduce 100M, grouped aggregate/join/wide-projection workloads, and broader mutation evidence |
...
| Spatial pruning | `just duckdb-adbc-storage-test`, `just duckdb-pgwire-workflow-test`, and `just duckdb-spatial-scan-profile` | one-table maintained-layout `ST_Intersects` adds planner-visible four-axis candidates while retaining the exact predicate. Two clean 10M mixed `POINT`/`LINESTRING`/`POLYGON` references on source `365c769` compare 25 ordered DuckLake files per layout. Every maintained WKB/bbox and native `GEOMETRY` query returns the same 100 exact pgwire hits. The runs scan one of 143/145 bbox and 100 native row groups; conservative compressed-byte upper bounds are 1.045%/1.016% and 1.266%. Five timed pgwire samples per layout pass the 500 ms p95, +128 MiB RSS, +256 MiB DuckDB-memory, and zero-spill budgets; both layouts compact 25 files to one. Broad analytical workloads and 100M remain open |
...
- COPY validates and maintains the explicit four-column bbox layout in DuckDB SQL;
  ...
  pgwire, and catalog contract. Automatic DDL, broader mutation policy and
  workloads, and 100M evidence remain open.
...
| M4 analytical performance | active; two 10M mixed-shape scan/resource runs pass | fail-closed bbox maintenance, exact recheck, edge-case correctness, compressed scan-byte accounting, and fragmented-file compaction evidence are implemented. Consecutive 10M maintained-bbox/native-geometry profiles cover points, lines, and polygons; return equal exact results; stay below 5% compressed bytes with at least 20x row-group reduction; pass committed pgwire latency, RSS, DuckDB-memory, and zero-spill budgets; and compact 25 files to one. Local 1.0 retains maintained WKB/bbox storage despite native files being about 45% smaller because only the maintained path has the required write/client contract. Remaining: broader mutation policy, grouped aggregates, bounded joins, wide projections, then two 100M profiles. |
