...
| Catalog contract | `just duckdb-catalog-contract-test` | client-neutral fixture executes DuckDB-derived table/column metadata, bounded geometry/geography sentinel lookup, geometry RowDescription/text/binary/NULL identity, unknown OID, and ordinary native `pg_type` behavior; named clients and broader PostgreSQL metadata remain open |
...
| Termination/restart | `just duckdb-termination-profile` | actual process forced-drain rolls back an explicit uncommitted row, preserves exact committed state on same-path restart, becomes queryable in 272 ms at smoke scale, and accepts a post-restart write; general write/commit interruption and release-catalog recovery remain open |
...
| M3 focused compatibility | active foundation | maintained SET/SHOW, AST `public` mapping, parsed quoted COPY targets, and a registered client-neutral fixture for DuckDB-derived table/column metadata, structural sentinel resolution, geometry RowDescription/text/binary/NULL identity, unknown OIDs, and ordinary native catalog behavior are verified. Focused encoder parity, panic-free nested errors, and ledger-pinned `0A000` simple/extended behavior for all 15 non-executable spatial cases also pass. Remaining: pinned named-client traces, broader catalog fixtures driven by those traces, subtype/SRID/dimension metadata, implementing release-required Rust-edge semantics, geography evidence, and broader generated encoder coverage. |
