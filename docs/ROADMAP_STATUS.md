...
| K0 minimal Kind topology | complete | clean source `be574bf` on rootless Podman runs one digest-addressed StatefulSet Pod with an ordered loopback trust-mode DuckDB server, worker edge, bootstrap, and sole mutual-TLS tiny-client ingress. Pinned psql/psycopg/OGR Jobs pass through the bridge; psycopg additionally proves copied-data WKB/NULL COPY plus reconnect readback. Direct worker TCP, plaintext, and certificate-free Jobs fail. Ordered replacement reconnects every expanded gate in 4.071 s; packaged mTLS/edge-key rotation denies the old client and reruns the copied-data and denial gates successfully |
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 enforcement/discovery plus direct role-aware REST now prove PostgreSQL types, role/legacy filtering, automatic cache revision invalidation, OpenAPI, and execution agreement; the pinned psycopg copied-data package slice passes | primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain blocked by DuckLake; obtain the official identity bundle, run copied-data psql/OGR/QGIS, expose shared monotonic epochs, and package the REST path |
...
| Packaged iroh ingress | `just kind-up-local`, `just kind-client-gates`, `just kind-restart-gate`, and `just kind-secret-rotation-gate` | one rootless-Podman Kind Pod runs the immutable server/bootstrap/worker/client bundle with fixed direct iroh routes and per-process owner-only keys. The server stays loopback/trust-only; the Service exposes only the mutual-TLS client. Pinned psql 18.3 and OGR 3.11.5 scalar gates pass; psycopg 3.2.13 additionally creates copied data, streams WKB/NULL COPY, reconnects, and verifies exact readback before/after replacement and key rotation. Direct worker TCP, plaintext, certificate-free, and rotated-old-certificate access fail. Ordered Pod replacement reconnects in 4.071 s |
...
- Rootless-Podman Kind mTLS tiny-client scalar smokes pass with psql 18.3 and
  GDAL/OGR 3.11.5. Pinned psycopg 3.2.13 additionally creates/reuses copied data,
  streams exact WKB/NULL rows with text COPY, closes/reconnects, and verifies
  exact scalar/spatial readback; direct worker TCP, plaintext, and
  ...
  actual-pgwire tests; copied psql/OGR/QGIS workflows have not yet run. Named QGIS,
