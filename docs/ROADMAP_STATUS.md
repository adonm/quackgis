...
| K0 minimal Kind topology | complete | clean source `63563cf` on rootless Podman runs runtime digest `sha256:48792f272e0a506866a1ab5c2c3b861d45c86ca9bccd33f3a756a9efae464806` in one StatefulSet Pod with an ordered loopback trust-mode DuckDB server, worker edge, bootstrap, and sole mutual-TLS tiny-client ingress. Pinned psql/psycopg/OGR Jobs pass through the bridge; psycopg proves copied-data WKB/NULL COPY plus reconnect, and OGR reads the same fixture through bounded SQL cursors with exact Point/NULL GeoJSON. Direct worker TCP, plaintext, and certificate-free Jobs fail. Ordered replacement reconnects every expanded gate in 5.042 s; packaged mTLS/edge-key rotation denies the old client and reruns both copied-data and denial gates successfully |
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 enforcement/discovery plus direct role-aware REST now prove PostgreSQL types, role/legacy filtering, durable schema/security epochs, capability-gated cache invalidation with exact fallback, OpenAPI, and execution agreement; pinned psycopg copied-data and OGR SQL-result read package slices pass | primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain blocked by DuckLake; obtain the official identity bundle, run copied-data psql/QGIS plus OGR direct discovery/COPY/no-FID, and package the REST/epoch path |
| H0 role-aware HTTP | role-aware direct-pgwire implementation complete, including shared epoch consumption where durable identity exists and exact revision fallback elsewhere | bounded HS256 JWT signature/issuer/audience/time/role validation drives one SCRAM authenticator, transaction-local role/claims, grant-filtered per-role catalog caches, role-specific OpenAPI, automatic live-schema/stale-role-cache replacement, reconnect-generation invalidation, and matching direct denial; next route through the tiny client and package/rotate multiple replicas |
...
| REST preview | `just rest-check`, `just rest-postgrest-smoke`, and `just rest-shared-epoch-smoke` | separate read-only sidecar pins `pg-rest-server` parser/schema inputs and proves HS256 validation/denial plus atomic key-file hot rotation with invalid-key readiness failure and old-key denial, owner-only file-backed authenticator password rotation, mismatch readiness failure, same-state database restart, automatic new-password reconnect without restarting REST, old-password denial, transaction-local role/claims cleanup, common-grant-filtered per-role PostgreSQL catalog/OpenAPI/direct denial, durable schema/security epoch and reconnect-generation invalidation in the checksum-pinned identity lane, exact role-filtered fallback in the signed lane, projection, typed filters, ordering, pagination, and escaped WKB through actual QuackGIS pgwire; signed epoch packaging, full PostgREST parity, writes, RPC, embedding, GeoJSON, tiny-client routing, and packaged deployment remain open |
...
| Packaged iroh ingress | `just kind-up-local`, `just kind-client-gates`, `just kind-restart-gate`, and `just kind-secret-rotation-gate` | one rootless-Podman Kind Pod runs the immutable server/bootstrap/worker/client bundle with fixed direct iroh routes and per-process owner-only keys. The server stays loopback/trust-only; the Service exposes only the mutual-TLS client. Pinned psql 18.3 scalar passes; psycopg 3.2.13 creates copied data, streams WKB/NULL COPY, reconnects, and verifies exact readback; OGR 3.11.5 then runs its extended SQL-result cursor lifecycle and verifies exact Point/NULL GeoJSON. Both copied-data gates pass before/after replacement and key rotation. Direct worker TCP, plaintext, certificate-free, and rotated-old-certificate access fail. Ordered Pod replacement reconnects in 5.042 s |
...
| TLS/credential rotation | `just duckdb-tls-rotation-profile`, `just kind-secret-rotation-gate`, and `just rest-postgrest-smoke` | actual-process TLS-required/SCRAM smoke verifies client hostname/trust, rejects plaintext and wrong trust, restarts with replacement certificate/password while preserving committed state, rejects old trust/password, and accepts a post-rotation write. Packaged Kind independently rotates its mutual-TLS and iroh keys, denies the old client certificate, and reconnects every gate. Direct REST atomically hot-rotates its HS256 file and reconnects after owner-only authenticator-password replacement plus same-state database restart; invalid/mismatched files fail readiness and old JWT/database credentials are denied. Packaged JWT/database rotation and production revocation remain open |
...
- The direct-pgwire REST preview validates HS256 signatures against one
  ...
  and database authorization remains authoritative. Shared monotonic epoch
  consumption remains open for packaged replicas.
- Rootless-Podman Kind mTLS tiny-client smoke passes with psql 18.3. Pinned
  ...
  in focused actual-pgwire tests. Copied-data psql, OGR direct discovery/COPY/
  no-FID, and QGIS remain open. Named QGIS, GeoServer, Martin, ORM, and BI
  workflows remain unqualified.
...
| M3 focused compatibility | C3/C4 complete; C5 active; H0 direct preview complete; psycopg copied-data and OGR SQL-result read slices complete | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user catalogs/origins/defaults/comments/NOT-NULL constraints/epochs, truthfully empty index discovery, generic role-aware spatial catalogs/probes/extents, immutable role sessions/context, common privilege enforcement, role-aware information schema, JWT-to-role REST/OpenAPI with shared epoch invalidation and exact fallback, packaged psycopg copied-data COPY/reconnect, and packaged OGR exact SQL-result Point/NULL readback pass focused/actual pgwire evidence. Remaining: upstream identity bundle; authoritative CRS/subtype/dimension metadata; an explicit client policy for DuckLake's missing key/index semantics; signed/packaged epochs; copied-data psql/QGIS, OGR direct discovery/COPY/no-FID, and role-aware REST through the packaged tiny client. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
...
| M5 Local 1.0 | active; K0 package boundary complete | immutable load-only runtime smoke, local read/write-capacity readiness, configured drain, authorized compaction, checksummed offline exact-path backup/restore, host TLS/SCRAM rotation, direct REST JWT/authenticator rotation, and the packaged direct iroh path exist. The rootless-Podman Kind package uses node-local digest aliases, retained storage, one ordered server/worker/bootstrap/client Pod, per-process owner-only edge keys, mutual-TLS ingress, and psql/psycopg/OGR Jobs. Psycopg COPY/reconnect and OGR SQL-result copied-data readback pass; direct/plaintext/certificate-free access fails; a 5.04 s ordered replacement reconnects all gates; packaged mTLS/edge-key rotation denies the old client certificate. Host I0 profiles remain authoritative for performance. Remaining local work: packaged I0 resource/hosted-relay reruns, PostgreSQL 18 catalog/RBAC and remaining copied-client M3 closure, catalog/control lifecycle, role-aware multi-replica REST/OpenAPI, packaged database/JWT rotation, published artifacts, production recovery/upgrade, mixed workload, and 24-hour soak. Final bundle selection is externally blocked on the not-yet-released DuckDB 1.5.5 (tentative 2026-07-20) and 2.0 (fall 2026) candidates required by M5. |
