...
| G0 offline PostGIS migration | planned; no product claim | first executable slice is a pinned source preflight plus one repeatable-read snapshot streamed through the packaged tiny client, with complete migrate/map/reject disposition and exact post-reopen verification |
