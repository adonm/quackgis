...
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS now owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. Durable schema/security epochs, role-aware REST/OpenAPI, pinned psycopg copied-data, and OGR SQL-result slices pass. Native catalog coverage now closes OGR's captured `pg_proc` PostGIS namespace lookup | rerun the pinned package and continue the psql/OGR catalog frontier from the last older-image `pg_class`/`pg_proc` stops; PostgreSQL 18.4 startup/version/recovery identity and failed/idle transaction cleanup now pass; QGIS read-only/binary cursor shapes remain. Primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain upstream gaps, not reasons to invent rows |
...
- Rootless-Podman Kind mTLS tiny-client smoke passes with psql 18.3. Pinned
  ...
  read-only/binary cursor shapes remain; GeoServer, Martin, ORM, and BI remain
  unqualified.
