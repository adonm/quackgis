...
| K0 minimal Kind topology | complete | rootless Podman runs one digest-addressed StatefulSet Pod with an ordered loopback trust-mode DuckDB server, worker edge, bootstrap, and sole mutual-TLS tiny-client ingress. Pinned psql/psycopg/OGR Jobs pass through the bridge; direct worker TCP, plaintext, and certificate-free Jobs fail. Ordered replacement reconnects in 4.01 s, and packaged mTLS/edge-key rotation denies the old client certificate before all gates reconnect |
