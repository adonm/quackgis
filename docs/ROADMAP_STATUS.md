...
| Offline recovery | `just duckdb-recovery-profile` | clean source `aba25e5` actual-process smoke stops at a declared 100-row/5,050-ID-sum/2,100-WKB-byte checkpoint, creates a 3-file checksum backup, writes 25 later rows to the original, deletes both durable paths, restores to the exact paths, and becomes queryable in 132 ms with the exact checkpoint, none of the later rows, and a successful post-recovery write. Online/relocated, release-scale, and disaster recovery remain open |
...
- Clean actual-process recovery on source `aba25e5` advances the existing storage
  unit test into an operator-path gate: the server stops at an exact 100-row
  ...
  paths are deleted; and exact-path restore becomes queryable in 131.74 ms with
