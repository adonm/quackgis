...
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS now owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. Durable schema/security epochs, role-aware REST/OpenAPI, pinned psycopg copied-data, and OGR SQL-result slices pass. Pinned native catalog coverage closes psql's captured `resolve_relation`, OGR's `pg_proc`/`column_structure`/zero-row `primary_key_columns`, and QGIS's `attribute_structure`; actual-pgwire executes QGIS's exact binary-cursor lifecycle with binary payload and `25006` enforcement | rerun the pinned package and continue psql at unsupported `relation_properties`, OGR after the older-image `pg_class`/`pg_proc` stops, and full headless QGIS. Primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain upstream gaps, not reasons to invent rows |
...
| Runtime supply chain | `just ducklake-pinned-source-check`, `just duckdb-pinned-ducklake-test`, and `just duckdb-runtime-offline-smoke` | clean source `c01d21e` produced local OCI digest `sha256:0ec89a73d284a5978b76bccd29242a421535be65b2a7f9617917aea19614c53d`; tracked DuckLake base/patch/tool/artifact pins, exact compile-time/config/file digest agreement, immutable image path, signed Spatial plus the pinned read-only DuckLake extension, no online install, network-disabled CLI load, all binary version probes, and server startup pass |
...
- Rootless-Podman Kind mTLS tiny-client smoke passes with psql 18.3. Pinned
  ...
  direct discovery/COPY/no-FID blocked in the last packaged retry at unavailable
  `pg_class`/`pg_proc`; the native catalog contract now executes the exact psql
  `resolve_relation`, captured `pg_proc` PostGIS namespace, and OGR/QGIS derived
  field-structure queries, pending a pinned-image rerun.
  ...
  failed COMMIT/ROLLBACK cleanup, and idle transaction end now pass. Exact QGIS
  read-only/binary cursor messages now pass native actual-pgwire with exact binary
  payloads and transaction-state enforcement; the pinned-image full QGIS workflow,
  GeoServer, Martin, ORM, and BI remain unqualified.
...
| M3 focused compatibility | C3/C4 complete; C5 active; H0 packaged preview complete; psycopg copied-data and OGR SQL-result read slices complete | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, bounded PostGIS routine identity, user catalogs/origins/defaults/comments/NOT-NULL constraints/epochs, empty index discovery, role-aware catalogs/information schema, common privilege enforcement, two-replica JWT-to-role REST/OpenAPI, packaged psycopg COPY/reconnect, and OGR SQL-result Point/NULL readback pass. The pinned native contract closes psql's captured `resolve_relation`, OGR's `pg_proc`, derived `column_structure`, and truthfully empty `primary_key_columns` queries, QGIS's derived `attribute_structure`, and QGIS's three frozen binary-cursor messages with exact wire payloads, rollback, failed-declare state, and write denial. Psql's next `relation_properties` shape remains unsupported; the older-image psql/OGR retries stopped at `pg_class`/`pg_proc`, and full QGIS qualification remains unrerun. Remaining: pinned package/client rerun, generic query-shape closure, key/index policy, authoritative CRS metadata, copied-data psql/QGIS, and OGR direct discovery/COPY/no-FID. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
