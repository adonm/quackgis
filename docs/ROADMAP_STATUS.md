...
| I0 early iroh transport | active; local-direct DuckDB slice implemented | shared `control/1`/`edge/1`, config-backed registered credential/worker, bounded signed leases and nonce replay cache, challenged transport-bound key proof, typed pgwire/cancellation streams, mandatory `none`, bounded loopback tiny client, role-bound startup, nested-TLS denial, and backend credential-challenge denial pass real local iroh endpoints. Actual DuckDB evidence now covers simple/extended and spatial queries, atomic COPY, rollback, cancellation/quarantine, and fresh reconnect; next broaden direct oracle parity, then run public-default/custom-relay correctness and resource profiles |
...
| Iroh local-direct seam | `just iroh-direct-smoke` | real relay-disabled local bootstrap, worker, and tiny-client endpoints issue one lease and multiplex two concurrent pgwire sessions; the loopback client path rejects nested TLS, structurally binds startup to the leased LOGIN role, permits typed cancellation, and refuses backend credential negotiation beyond `AuthenticationOk`. The deterministic backend is a fake trust-mode pgwire seam, so DuckDB semantics and resource budgets remain open |
| Iroh DuckDB direct path | `just iroh-duckdb-smoke` | the same real local endpoints reach a fresh official-DuckLake QuackGIS worker and match a direct scalar oracle; typed parameters and spatial SQL execute, two-row text COPY publishes exact count/sum, rollback adds zero rows, pgwire cancellation returns `57014`, the cancelled session is quarantined, and a fresh tunneled session observes the exact committed rows. Relay, scale, and resource claims remain open |
...
- The I0 binaries load owner-only key files and validate omitted-public versus
  ...
  The native direct gate covers scalar/typed/spatial queries, COPY publication,
  rollback, cancellation/quarantine, and fresh reconnect. Broader pgwire parity,
  public/custom relay paths, resource budgets, packaged local-boundary protection,
  and role-catalog preauthentication remain open.
