...
| C0 focused clients | active foundation; client-neutral catalog fixture established | run pinned psql and psycopg first, then OGR and headless QGIS |
...
- COPY incrementally decodes bounded Arrow batches into one staging ADBC stream;
  ...
  0.200; optimization and a passing reference remain open. A configurable
...
| M2 streaming ingest | active; implementation majority complete | configurable pre-body frontend-frame rejection, incremental bounded parser, exact Arrow batch byte splitting, staging ADBC stream, atomic publication, text escapes, >20 MiB/220k-row regression, synchronous malformed-final-row and oversized-decoded-chunk cleanup, abort zero-row tests, scalar/NULL/WKB reopen, compaction, and metrics are implemented. The same direct-ADBC/pgwire generator and exact oracle pass at 1M local scale with 64 MiB RSS delta and 0.272 throughput ratio. The first clean 10M run measures 0.200 and fails the required 0.50 ratio. Remaining: COPY optimization and passing reference plus dependency-limited idle-wait error delivery. |
| M3 focused compatibility | active foundation | maintained SET/SHOW, AST `public` mapping, parsed quoted COPY targets, and a registered client-neutral fixture for DuckDB-derived table/column metadata, exact-shape seven-field sentinel resolution, geometry/geography RowDescription/text/binary/NULL identity, unknown OIDs, near-miss non-interception, and ordinary native catalog behavior are verified. Focused encoder parity, panic-free nested errors, and ledger-pinned `0A000` simple/extended behavior for all 15 non-executable spatial cases also pass. Remaining: pinned named-client traces, broader catalog fixtures driven by those traces, subtype/SRID/dimension metadata, implementing release-required Rust-edge semantics, and broader generated encoder coverage. |
...
| M5 Local 1.0 | active foundation | immutable load-only runtime smoke, opt-in process liveness and pgwire-bind/read-only DuckLake readiness, configured drain, authorized/audited compaction, checksummed offline exact-path backup/restore, actual-process TLS/SCRAM/plaintext-denial/restart-rotation evidence, and explicit TLS-required startup policy exist. A clean actual-process profile on source `59c1a381` proves forced-drain rollback of an explicit uncommitted transaction, exact same-path restart in 135 ms at smoke scale, and a successful post-restart write. The initial [minimal Kind topology](../deploy/kind/README.md) renders one TLS-required StatefulSet, retained local storage, generated Secrets, probes, and digest-pinned psql/psycopg/OGR Jobs; it has static evidence only until runnable client/runtime images and Kind are available. Host profiles remain authoritative for performance. Remaining: real cluster/client execution and packaged rotation, write-capacity readiness SLO, published artifacts, online/relocated production recovery, upgrades, general write/commit interruption evidence, mixed workload, and 24-hour soak. |
