...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete; C5 common enforcement now combines schema USAGE, table ownership, direct/inherited/PUBLIC grants, and the legacy outer ceiling for maintained reads/writes/COPY/maintenance through actual pgwire | add relational role/membership catalogs and privilege inquiry from the same engine, then constraints/indexes/defaults/comments/spatial/information-schema work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
...
| Auth/policy | real CLI SCRAM, table allowlist, role-session, request-context, and effective-role grant/denial cases in pgwire workflow | bounded immutable role graph; original-login set-option traversal; name-typed identity; role/context lifecycle and prepared invalidation. Common schema/table ownership/direct/inherited/PUBLIC decisions gate maintained operations behind the legacy outer ceiling. Role catalogs/inquiry remain open |
...
| M3 focused compatibility | C3/C4 complete; C5 active | core profile/oracle, durable DuckLake identity, exact psql/OGR/QGIS traces, built-in/spatial/logical-database identity, user catalogs/origins/epochs, immutable role sessions/context, and cancellation isolation pass. Common configured privileges now enforce maintained operations through actual pgwire behind the legacy ceiling. Remaining: upstream identity bundle; role/membership catalogs and inquiry; traced constraint/index/default/comment/spatial catalogs and privilege-aware information schema; REST epoch consumption; richer provenance; copied-data clients and role-aware REST. Mutable role DDL, RLS, and HTTP mutations remain follow-on work. |
