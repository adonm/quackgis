...
| C0 PostgreSQL compatibility | C3 implementation complete in the exact-1.5.4 gated lane; C4 complete with bounded immutable role/owner/grant configuration, graph validation, per-connection session/effective identity, set-option role assumption, local role lifecycle, prepared invalidation, bounded transaction-local request context, and cancellation/quarantine/fresh-session evidence through actual pgwire | implement C5 role catalogs, common privileges, constraints/indexes/defaults/comments/spatial/information-schema work; obtain official identity bundle before release and run copied-data clients/role-aware REST |
