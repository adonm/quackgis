...
| C0 PostgreSQL compatibility | C3/C4 implementation and C5 enforcement/discovery pass; QuackGIS now owns the exact-source/patch/artifact-pinned identity lane and runtime packaging policy. Durable schema/security epochs, role-aware REST/OpenAPI, pinned psycopg copied-data, and OGR SQL-result slices pass. Pinned native catalog coverage closes OGR's captured `pg_proc`, derived `column_structure`, and zero-row `primary_key_columns` queries plus QGIS's derived `attribute_structure`; actual-pgwire executes QGIS's exact binary-cursor lifecycle with binary payload and `25006` enforcement | rerun the pinned package and continue the psql/OGR catalog frontier from the last older-image `pg_class`/`pg_proc` stops; run full headless QGIS qualification. Primary/unique/foreign-key indexes and authoritative CRS/subtype/dimension metadata remain upstream gaps, not reasons to invent rows |
...
- Rootless-Podman Kind mTLS tiny-client smoke passes with psql 18.3. Pinned
  ...
  `pg_class`/`pg_proc`; the native catalog contract now executes the exact
  captured `pg_proc` PostGIS namespace and OGR/QGIS derived field-structure
  queries, pending a pinned-image rerun.
