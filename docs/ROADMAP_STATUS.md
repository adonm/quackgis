...
| P0 M4 host profiles | active; two 10M and two 100M mixed-shape scan/resource runs pass | add grouped aggregate, bounded join, wide-projection, and broader mutation evidence |
...
| Spatial pruning | `just duckdb-adbc-storage-test`, `just duckdb-pgwire-workflow-test`, and `just duckdb-spatial-scan-profile` | one-table maintained-layout `ST_Intersects` adds planner-visible four-axis candidates while retaining the exact predicate. Two clean 10M mixed `POINT`/`LINESTRING`/`POLYGON` references on source `365c769` and two clean 100M references on `bd4f0ab` compare 25 ordered DuckLake files per layout. Every maintained WKB/bbox and native `GEOMETRY` query returns the same 100 exact pgwire hits. At 100M both layouts scan 1/825 row groups with 0.128–0.129% conservative compressed-byte upper bounds. Five timed pgwire samples per layout pass the 250/500/750 ms p50/p95/p99, +128 MiB RSS, +256 MiB DuckDB-memory, and zero-spill budgets; compaction reduces 25 files to 7 bbox/4 native. Broad analytical workloads remain open |
...
- COPY validates and maintains the explicit four-column bbox layout in DuckDB SQL;
  ...
  profiler reports N scans. Two clean 10M mixed `POINT`/`LINESTRING`/`POLYGON`
  runs on source `365c769` pass exact pgwire, 5% scan-byte, 20x row-group, 500 ms
  p95, +128 MiB process RSS, +256 MiB DuckDB-memory, zero-spill, and 2x compaction
  gates. Observed p95 is 15.46–18.50 ms for maintained bbox and 19.41–20.52 ms
  for native geometry; process RSS grows by at most 1.90 MiB and sampled DuckDB
  memory by 10.10 MiB. Both layouts compact from 25 files to one without result
  changes. Native geometry files are about 45% smaller, but Local 1.0 retains the
  maintained WKB/bbox representation because it is the proven COPY, mutation,
  pgwire, and catalog contract. Two clean 100M runs on source `bd4f0ab` then load
  each pair of layouts in 23.83–23.92 seconds, scan 1/825 row groups and at most
  0.129% compressed bytes, observe 17.59–19.90 ms p50 and 19.40–20.32 ms
  p95/p99, grow process RSS by at most 15.44 MiB and sampled DuckDB memory by at
  most 13.20 MiB, use zero temporary storage, and compact 25 files to 7 bbox/4
  native files without result changes. Automatic DDL, broader mutation policy,
  grouped aggregates, bounded joins, and wide projections remain open.
...
| M4 analytical performance | active; two 10M and two 100M mixed-shape scan/resource runs pass | fail-closed bbox maintenance, exact recheck, edge-case correctness, compressed scan-byte accounting, and fragmented-file compaction evidence are implemented. Consecutive 10M and 100M maintained-bbox/native-geometry profiles cover points, lines, and polygons; return equal exact results; stay below 5% compressed bytes with at least 20x row-group reduction; pass committed load, pgwire p50/p95/p99, RSS, DuckDB-memory, and zero-spill budgets; and at least halve 25 files. Local 1.0 retains maintained WKB/bbox storage despite native files being about 45% smaller because only the maintained path has the required write/client contract. Remaining: broader mutation policy, grouped aggregates, bounded joins, and wide projections. |
