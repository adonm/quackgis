...
| PostgreSQL catalog snapshot/epoch | active/Rust edge | `pg18-column-core-v1`; DuckDB 1.5.4 multi-process gate proves durable DuckLake table UUID/ID and column field IDs; upstream committed-snapshot `ducklake_column_info` boundary selected; checksum-pinned development 1.5 port passes DuckLake and QuackGIS lifecycle gates; broad metadata denied | transactional PostgreSQL OID/attribute registry, immutable snapshot, commit/rollback invalidation; upstream acceptance and signed official bundle remain release gates |
