...
| role-aware REST/OpenAPI | active/stateless pgwire client | bounded HS256 JWT validation and role allowlist; one SCRAM authenticator; transaction-local role/claims; common catalog/grant-filtered per-role cache, OpenAPI, direct denial, live schema revision, and stale-wide-cache repair through actual pgwire | shared monotonic epochs, tiny-client routing, key/database rotation, packaged replicas |
