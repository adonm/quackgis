...
| exact bbox recheck | native query + Rust AST edge | one-table maintained-layout `ST_Intersects` injects four conservative candidates while retaining the exact predicate; unit edge cases and pgwire exact oracles pass. Two clean 10M and two 100M mixed point/line/polygon references compare maintained WKB/bbox and native `GEOMETRY`, with equal exact counts, at least 20x row-group reduction, conservative compressed scan bytes below 5%, 19.40–20.32 ms 100M pgwire p95/p99, at most 15.44 MiB process-RSS growth, 13.20 MiB DuckDB-memory growth, and zero spill | grouped aggregate, bounded join, and wide-projection workloads |
