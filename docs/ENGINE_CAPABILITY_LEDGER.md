...
| roles/session/request context | active/Rust edge | bounded immutable role/owner/grant config; cycle/LOGIN validation; name-typed session/effective identity; original-login set-option assumption; SET/SET LOCAL/NONE/RESET; transaction cleanup and prepared invalidation; exact 16 KiB transaction-local `request.jwt.claims` set/get with commit/failed-rollback cleanup through actual pgwire | focused cancellation/quarantine context oracle, then role catalogs and common privilege enforcement |
