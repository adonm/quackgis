...
| QGIS read-only | blocked on QuackGIS catalog surface | digest-pinned offscreen QGIS 3.44.11 succeeds against PostgreSQL 18.4/PostGIS for layer open, fields, CRS, privilege/owner checks, count, 3D extent, and binary-cursor identify; 32 statements/26 unique families frozen | implement traced catalogs/privilege functions/spatial metadata and run the same workflow against QuackGIS; add filter/render |
