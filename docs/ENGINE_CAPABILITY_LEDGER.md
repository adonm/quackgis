...
| PostgreSQL catalog snapshot/epoch | active/Rust edge | `pg18-column-core-v1`; DuckDB 1.5.4 multi-process gate proves durable DuckLake table UUID/ID and column field IDs; checksum-pinned development `ducklake_column_info` lane now allocates protected durable namespace/relation/row-type OIDs and attribute numbers, retains tombstones, serializes concurrent commit/reconcile pairs, covers public create APIs, validates unconstrained-registry invariants, and advances an identity-fingerprint epoch across commit/rollback/rename/add/reopen/drop-recreate/non-public-schema cases | obtain durable empty-schema identity; project `pg_class`/`pg_attribute` and row types; consume epochs in immutable snapshots/caches; add RowDescription origins; upstream acceptance and signed official bundle remain release gates |
| core `pg_catalog`/`information_schema` | partial/Rust edge + native relational evaluation | explicit/implicit structural mapping to protected namespace/database/type/range/collation/owner-role views; stable logical-database identity plus `current_database`/`current_schema`/`current_schemas` with PostgreSQL `name`/`name[]` results; 24 exact PostgreSQL 18 built-ins plus PostGIS-shaped geometry/geography scalar+array rows; complete namespace/owner/array/collation references; provenance-bound wire/OID parameter hints; all unimplemented catalog routing and unsupported projection shapes fail closed | broader built-ins, user class/attribute/constraint surfaces, wildcard/nested/set provenance, `reg*`, RowDescription origins, privilege-aware discovery |
