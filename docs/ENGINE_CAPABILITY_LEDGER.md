...
| PostgreSQL catalog snapshot/epoch | blocked/Rust edge | `pg18-column-core-v1`; DuckDB 1.5.4 multi-process gate proves durable DuckLake table UUID/ID and column field IDs; upstream committed-snapshot `ducklake_column_info` boundary selected and main-branch prototype verified; broad metadata denied | upstream acceptance and pinned-runtime public identity contract, transactional PostgreSQL OID/attribute registry, immutable snapshot, commit/rollback invalidation |
