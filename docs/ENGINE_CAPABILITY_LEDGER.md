...
| exact bbox recheck | native query + Rust AST edge | one-table maintained-layout `ST_Intersects` injects four conservative candidates while retaining the exact predicate; unit edge cases and pgwire exact oracles pass. Clean 100k/1M and two 10M profiles compare maintained WKB/bbox and native `GEOMETRY`, with equal exact counts, at least 20x row-group reduction, and conservative compressed scan bytes below 5% | representative non-point shapes, query latency/resources, and 100M profiles |
| layout/locality maintenance | partial native SQL | COPY computes four reserved bbox columns; numbered-bound/NULL geometry UPDATE atomically refreshes them with malformed/rollback/reopen evidence; direct INSERT, arbitrary geometry expressions, and reserved writes fail closed. The point profile compacts both 10M layouts from 25 files to one without changing exact results | broader geometry mutation/compaction policy and non-point evidence |
