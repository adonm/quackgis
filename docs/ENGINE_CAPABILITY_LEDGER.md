...
| core `pg_catalog`/`information_schema` | C3 complete; C5 discovery active in Rust/native lane | baseline namespace/database/type/range/collation/owner-role views plus development-only registry-backed `pg_namespace`/`pg_class`/`pg_attribute`/`pg_attrdef`/`pg_description`/composite `pg_type`; DuckLake defaults and table/column comments resolve through PostgreSQL-typed `pg_get_expr`/`col_description`/`obj_description` under effective-role and legacy filtering; role-bound `schemata`/`tables`/`columns` and table/column grant views; exact maintained `pg18-column-core-v1` descriptions; scalar/spatial/registered-object and information-schema wire types; search-path-aware strict/nullable `regclass`/`regtype`/`regnamespace`/`regrole`, OID/text casts and `format_type`; unsupported types/features fail closed | signed upstream identity bundle for release; constraints/indexes and richer provenance remain C5/M3 work |
