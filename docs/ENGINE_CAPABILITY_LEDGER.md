...
| core `pg_catalog`/`information_schema` | C3 complete; C5 discovery active in Rust/native lane | baseline namespace/database/type/range/collation/owner-role views plus development-only registry-backed `pg_namespace`/`pg_class`/`pg_attribute`/composite `pg_type`; role-bound `schemata`/`tables`/`columns` and table/column grant views; exact maintained `pg18-column-core-v1` descriptions; scalar/spatial/registered-object and information-schema wire types; search-path-aware strict/nullable `regclass`/`regtype`/`regnamespace`/`regrole`, OID/text casts and `format_type`; unsupported types/features fail closed | signed upstream identity bundle for release; constraints/indexes/defaults/comments and richer provenance remain C5/M3 work |
