...
| core `pg_catalog`/`information_schema` | partial/Rust edge + native relational evaluation | explicit/implicit structural mapping to protected namespace/type/range/collation/owner-role views; 24 exact PostgreSQL 18 built-ins plus PostGIS-shaped geometry/geography scalar+array rows; complete namespace/owner/array/collation references; provenance-bound wire/OID parameter hints; all unimplemented catalog routing and unsupported projection shapes fail closed | broader built-ins, class/attribute/database/constraint surfaces, wildcard/nested/set provenance, `reg*`, RowDescription origins, privilege-aware discovery |
