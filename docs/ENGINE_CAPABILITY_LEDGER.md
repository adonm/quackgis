...
| core `pg_catalog`/`information_schema` | C3 complete; C5 discovery active in Rust/native lane | baseline namespace/database/type/range/collation/owner-role views plus development-only registry-backed `pg_namespace`/`pg_class`/`pg_attribute`/`pg_attrdef`/`pg_description`/`pg_constraint`/`pg_index`/composite `pg_type`; defaults/comments and stable NOT-NULL constraint OIDs resolve through PostgreSQL-typed helpers under effective-role and legacy filtering; `pg_index` is explicitly empty because DuckLake supports no indexes or key constraints; role-bound information schema, registered objects, casts, and `format_type` remain fail-closed | signed upstream identity bundle; spatial metadata and richer provenance; upstream primary/unique/foreign-key/index support or an explicit client limitation policy |
...
| GDAL/OGR | partial | pinned 3.11.5 TLS/SCRAM scalar Kind smoke plus digest-pinned PostgreSQL 18.4/PostGIS copied-point oracle; class/attribute/default/comment and truthfully empty index catalogs exist, but QuackGIS discovery still reaches unsupported spatial/SRID paths and cannot expose a DuckLake primary key | spatial metadata, read + streaming COPY copied-data test, and explicit no-FID behavior |
