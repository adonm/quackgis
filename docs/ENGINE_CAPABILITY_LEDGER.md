...
| exact bbox recheck | native query + Rust AST edge | one-table maintained-layout `ST_Intersects` injects four conservative candidates for bounded literal envelope/text and numbered-bound WKB probes while retaining the exact predicate; unit edge cases and pgwire exact oracles pass. The registered 100k profile compares maintained WKB/bbox and native `GEOMETRY`, with equal exact counts and 1/25 row groups scanned for both | compressed scan bytes, fragmented-file compaction, and current 10M then 100M profiles |
