...
| role-aware REST/OpenAPI | active/stateless pgwire client | bounded HS256 JWT validation and role allowlist; one SCRAM authenticator; transaction-local role/claims; common catalog/grant-filtered per-role cache, OpenAPI, and direct denial through actual pgwire | schema/security epoch invalidation, tiny-client routing, key/database rotation, packaged replicas |
