...
| Constructors and text/WKB IO | ✅ pgwire starter + SQL fixtures | `ST_GeomFromText`, `ST_GeomFromEWKT`, `ST_Point`, `ST_MakePoint`, `ST_MakeEnvelope`, `ST_AsText`, `ST_AsEWKT`, `ST_AsHEXEWKB`, WKB/EWKB client payloads |
...
| Measures/accessors | ✅ SQL fixtures; starter pgwire subset for area/distance/length/perimeter, `ST_X`/`ST_Y`, `ST_XMin`/`ST_YMin`/`ST_XMax`/`ST_YMax`, `GeometryType`/`ST_GeometryType`, `ST_NDims`/`ST_CoordDim`/`ST_Dimension`, `ST_NPoints`/`ST_NumPoints`, `ST_StartPoint`/`ST_EndPoint`/`ST_PointN`, polygon ring accessors, `ST_NumGeometries`/`ST_GeometryN`, `ST_IsEmpty`, `ST_IsValid`, `ST_IsClosed`, `ST_IsRing`, and helper metadata | Area, length, perimeter, distance, bbox accessors, dimension/`ST_Zmflag`, collection/validity helpers |
...
| Editing/affine/simplify | ✅ SQL fixtures with documented deltas; pgwire renderer helpers plus starter pgwire affine/editing subset | `ST_Reverse`, `ST_FlipCoordinates`, `ST_Translate`, `ST_Scale`, `ST_Force2D`, `ST_CurveToLine`, `ST_HasArc`, and `ST_Simplify`; broader normalize/set-remove/add-point/rotate/affine/snap-to-grid coverage stays fixture/client-trace promoted |
