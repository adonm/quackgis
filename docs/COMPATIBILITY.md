...
| GDAL/OGR | pinned 3.11.5 reads the psycopg-created copied-data fixture through the mutual-TLS tiny client in both its unmodified SQL-result cursor lifecycle and direct discovery path, with exact Point/NULL GeoJSON and truthful no-FID behavior. It also appends a Point/NULL GeoJSON fixture to a separate predeclared table with `PG_USE_COPY=YES`; bounded plain PostGIS EWKB hex is accepted only for classified spatial fields, and a fresh connection verifies exact committed values. The pinned native catalog contract executes the captured `pg_proc` namespace, derived `column_structure`, and zero-row `primary_key_columns` queries with exact types/defaults/comments and no invented key. OGR table creation and authoritative CRS qualification remain open |
| QGIS | pinned offscreen 3.44.11-Solothurn passes the optional Kind gate through mutual TLS. A read-only copied-data query layer uses an explicit `id` key, discovers exact `id`/`name` fields, counts two rows, reads exact Point/NULL features through QGIS's binary cursor, reports the exact Point extent, applies expression and provider subset filters, identifies the Point with a spatial viewport request, and renders a non-empty 128×128 image. The viewport rewrite is limited to a literal SRID-0 envelope over a maintained WKB column and retains an exact `ST_Intersects` predicate. The native lane also proves QGIS-shaped privilege/recovery, derived `attribute_structure`, startup/version, transaction cleanup, binary payload, and `25006` write denial. Direct ordinary-table open remains unsupported without a real primary/unique key; QuackGIS does not invent one from DuckLake metadata, and authoritative CRS metadata remains open |
