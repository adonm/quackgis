...
| `quackgis-rest` | signed-HS256-JWT read-only preview maps a bounded role claim, uses one SCRAM authenticator with transaction-local role/claims, and discovers each role's exposed tables/columns and PostgreSQL types through maintained `information_schema`; projection/filter/order/pagination, role-aware OpenAPI/direct denial, and context cleanup pass actual pgwire, while automatic epoch invalidation, rotation, tiny-client routing, and packaging remain open |
