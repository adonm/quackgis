...
| GDAL/OGR | 3.11.5 mutual-TLS scalar smoke passes through the tiny client in Kind; traced generic spatial metadata/SRID/extent query shapes pass focused actual pgwire, while the copied-data client workflow and no-FID behavior remain open |
...
| `quackgis-rest` | signed-HS256-JWT read-only preview maps a bounded role claim, uses one SCRAM authenticator with transaction-local role/claims, and discovers each role's exposed tables/columns and PostgreSQL types through maintained `information_schema`; projection/filter/order/pagination, role-aware OpenAPI/direct denial, context cleanup, automatic role-filtered schema/security revision invalidation, and atomic HS256 key rotation with old-key denial pass actual pgwire, while shared monotonic epochs, authenticator/database rotation, tiny-client routing, and packaging remain open |
