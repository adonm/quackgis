...
| GDAL/OGR | pinned 3.11.5 reads the psycopg-created copied-data fixture through the mutual-TLS tiny client and its unmodified SQL-result cursor lifecycle, with exact Point/NULL GeoJSON. The pinned native catalog contract executes the captured `pg_proc` namespace and derived `column_structure` queries with exact types/defaults/comments and no invented uniqueness. The last older-image packaged direct-discovery retry stopped at `pg_proc`/`pg_class`, so a pinned-image rerun and downstream COPY/no-FID/CRS qualification remain open |
