...
# plan a copy-then-remove quarantine outside the live data path:
just orphan-quarantine-plan prefix=.tmp/orphan-quarantine min_age_seconds=86400 show_paths=true
```

Inventory always exits instead of starting pgwire. A strict positive age excludes
recent/in-flight files. By default output contains only `dry_run`, cutoff age,
and candidate count; paths require explicit opt-in. Only unreferenced `.parquet`
objects are candidates—live data/delete files and scheduled-deletion references
are subtracted by the DuckLake maintenance API. SQLite refuses a missing catalog.
PostgreSQL/S3 scans the shared data path and uses references from every
multicatalog catalog, so operators must treat its scope as global.

Quarantine is a separate explicit offline step. `just orphan-quarantine-plan`
prints the source→quarantine mapping without moving objects. `just
orphan-quarantine-apply` requires a destination outside the live data prefix,
copies each still-old candidate to that prefix, rechecks the catalog immediately
before removing the source, refuses to overwrite existing quarantine objects, and
never removes catalog rows. Use it only after quiescing writers and taking a
restore point; permanent deletion from quarantine remains a platform-retention
decision, not a QuackGIS automatic vacuum.
...
and local quarantine mechanics, not deletion safety on Kind or managed storage.
