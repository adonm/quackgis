...
The database URL must not contain a password. Direct mode additionally requires
owner-only `QUACKGIS_REST_DATABASE_PASSWORD_FILE`. Packaged
`QUACKGIS_REST_DATABASE_AUTH_MODE=edge-preauthenticated` rejects that file and a
CA, requires a literal loopback database host, and is valid only behind an exact
credential-bound edge lease plus loopback role-catalog preauthentication. The
authenticator must have configured set-option membership in every listed API
role. See [REST_API.md](REST_API.md) for the read-only contract,
credential-rotation order, and load-balancer trust boundary.
