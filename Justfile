...
# Verify candidate digests, exact core identity, offline loading, DuckLake functions, and Spatial CRS.
native-bundle-candidate-test prepared=".tmp/native-bundle":
    @prepared_arg='{{prepared}}'; prepared_arg="${prepared_arg#prepared=}"; \
    mise x aqua:Kitware/CMake@4.3.3 aqua:ninja-build/ninja@1.13.2 -- python3 scripts/build_native_bundle.py --prepared "$prepared_arg" --test
...
# Opt-in network check for newer released/compatible upstream refs and review drift.
native-upstream-check:
    python3 scripts/check_native_upstreams.py
...
# Prove checksum-verified exact-path backup/restore through actual server processes.
duckdb-recovery-profile level="smoke" out=".tmp/duckdb-recovery/manifest.json" environment="host_process" driver=duckdb_adbc_driver:
    @set -eu; level_arg='{{level}}'; out_arg='{{out}}'; environment_arg='{{environment}}'; driver_arg='{{driver}}'; \
    level_arg="${level_arg#level=}"; out_arg="${out_arg#out=}"; environment_arg="${environment_arg#environment=}"; driver_arg="${driver_arg#driver=}"; \
    if [ ! -f "$driver_arg" ]; then echo 'DuckDB ADBC driver is missing; run `mise run duckdb-bootstrap`' >&2; exit 2; fi; \
    driver_arg="$(realpath "$driver_arg")"; duckdb_home_arg="$(realpath -m '{{duckdb_home}}')"; out_arg="$(realpath -m "$out_arg")"; \
    HOME="$duckdb_home_arg" QUACKGIS_DUCKDB_ADBC_DRIVER="$driver_arg" QUACKGIS_PROFILE_OUT="$out_arg" \
      QUACKGIS_EVIDENCE_LEVEL="$level_arg" QUACKGIS_EXECUTION_ENVIRONMENT="$environment_arg" \
      cargo test -p quackgis-server --release --test roadmap_profiles offline_recovery_profile -- --ignored --exact --nocapture --test-threads=1; \
    python3 scripts/evidence_manifest_check.py "$out_arg"

# Required fast actual-process backup/restore evidence.
duckdb-recovery-smoke:
    just duckdb-recovery-profile level=smoke out=.tmp/duckdb-recovery/smoke.json

# Run the M5 COPY/mutation/read/cancel/compaction/restart workload; reference is fixed at 24 hours.
duckdb-mixed-release-profile level="smoke" duration="" out=".tmp/duckdb-mixed-release/manifest.json" environment="host_process" driver=duckdb_adbc_driver:
    @set -eu; level_arg='{{level}}'; duration_arg='{{duration}}'; out_arg='{{out}}'; environment_arg='{{environment}}'; driver_arg='{{driver}}'; \
    level_arg="${level_arg#level=}"; duration_arg="${duration_arg#duration=}"; out_arg="${out_arg#out=}"; environment_arg="${environment_arg#environment=}"; driver_arg="${driver_arg#driver=}"; \
    if [ ! -f "$driver_arg" ]; then echo 'DuckDB ADBC driver is missing; run `mise run duckdb-bootstrap`' >&2; exit 2; fi; \
    driver_arg="$(realpath "$driver_arg")"; duckdb_home_arg="$(realpath -m '{{duckdb_home}}')"; out_arg="$(realpath -m "$out_arg")"; \
    if [ -n "$duration_arg" ]; then export QUACKGIS_PROFILE_DURATION_SECONDS="$duration_arg"; fi; \
    HOME="$duckdb_home_arg" QUACKGIS_DUCKDB_ADBC_DRIVER="$driver_arg" QUACKGIS_PROFILE_OUT="$out_arg" \
      QUACKGIS_EVIDENCE_LEVEL="$level_arg" QUACKGIS_EXECUTION_ENVIRONMENT="$environment_arg" \
      cargo test -p quackgis-server --release --test roadmap_profiles mixed_release_workload_profile -- --ignored --exact --nocapture --test-threads=1; \
    python3 scripts/evidence_manifest_check.py "$out_arg"

# Required reduced M5 mixed release workload evidence.
duckdb-mixed-release-smoke:
    just duckdb-mixed-release-profile level=smoke duration=3 out=.tmp/duckdb-mixed-release/smoke-s3.json
