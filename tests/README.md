...
- `fixtures/postgresql18_compatibility_profile.json` freezes the first target
  ...
  The profile now also records source-executable captured query families: QGIS
  `attribute_structure`, OGR `column_structure`, and OGR's truthfully empty
  `primary_key_columns`. The pinned identity test reads their SQL from the trace
  fixtures and proves exact wire types and rows; all other trace statements
  remain targets unless separately named as evidence.
