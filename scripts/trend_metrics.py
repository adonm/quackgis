...
def render_dashboard(rows: list[dict[str, Any]]) -> str:
    ...
    body = [
        ...
        "candidate narrowing, native DML/compaction counters, writer conflicts, and",
    ...
    body.extend(
        [
            ...
            f"| Client real-data counts | `osm_postgis_parity` | {sum(1 for row in latest if present(row, 'qgis_points_feature_count') or present(row, 'qgis_lines_feature_count') or present(row, 'qgis_multipolygons_feature_count'))} check(s) |",
